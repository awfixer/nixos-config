...
use awm::animation::Clock;
use awm::layout::{ActivateWindow, AddWindowTarget, LayoutElement as _, Options, SizingMode};
use awm::render_helpers::{RenderCtx, RenderTarget};
use awm_config::{Color, OutputName, PresetSize};
...
pub struct Layout {
    ...
    clock: Clock,
    layout: awm::layout::Layout<TestWindow>,
    start_time: Duration,
    ...
    pub fn new(args: Args) -> Self {
        ...
        let options = Options {
            layout: awm_config::Layout {
                focus_ring: awm_config::FocusRing {
                    off: true,
                    ..Default::default()
                },
        ...
        let mut layout = awm::layout::Layout::with_options(clock.clone(), options);
