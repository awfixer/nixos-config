...
use crate::awm::{CastTarget, ClientState, LockState, State};
...
impl CompositorHandler for State {
    ...
    fn new_subsurface(&mut self, surface: &WlSurface, parent: &WlSurface) {
        ...
        if let Some(output) = self.awm.output_for_root(&root) {
    ...
    fn commit(&mut self, surface: &WlSurface) {
        ...
        if surface == &root_surface {
            ...
            if let Entry::Occupied(entry) = self.awm.unmapped_windows.entry(surface.clone()) {
                if is_mapped(surface) {
                    ...
                    } = state
                    {
                        ...
                        let output =
                            output.filter(|o| self.awm.layout.monitor_for_output(o).is_some());

                        // Check that the workspace still exists.
                        let workspace_id = workspace_name
                            .as_deref()
                            .and_then(|n| self.awm.layout.find_workspace_by_name(n))
                            .map(|(_, ws)| ws.id());
                    ...
                    let output = self.awm.layout.add_window(
                        mapped,
                        target,
                        width,
                        height,
                    ...
                    if let Some((mapped, _)) = self.awm.layout.find_window_and_output(surface) {
                    ...
                    if let Some(output) = output {
                        self.awm.layout.start_open_animation_for_window(&window);

                        let new_focus = self.awm.layout.focus().map(|m| &m.window);
            ...
            if let Some((mapped, output)) = self.awm.layout.find_window_and_output(surface) {
                let window = mapped.window.clone();
                let output = output.cloned();
                ...
                if !is_mapped {
                    ...
                    self.awm.unmapped_windows.insert(surface.clone(), unmapped);
                ...
                self.awm.window_mru_ui.update_window(&self.awm.layout, id);
                self.awm.layout.update_window(&window, serial);

                // Move the toplevel according to the attach offset.
                if let Some(delta) = buffer_delta {
                    if delta.x != 0 || delta.y != 0 {
                        let (x, y) = delta.to_f64().into();
                        self.awm.layout.move_floating_window(
                            Some(&window),
        ...
        let root_window_output = self.awm.layout.find_window_and_output(&root_surface);
        if let Some((mapped, output)) = root_window_output {
            ...
            self.awm
                .window_mru_ui
                .update_window(&self.awm.layout, mapped.id());
        ...
        if let Some(popup) = self.awm.popups.find_popup(&root_surface) {
        ...
        if matches!(
            &self.awm.cursor_manager.cursor_image(),
        ...
        if matches!(&self.awm.dnd_icon, Some(icon) if icon.surface == root_surface) {
            let dnd_icon = self.awm.dnd_icon.as_mut().unwrap();
        ...
        for (output, state) in &self.awm.output_state {
            if let Some(lock_surface) = &state.lock_surface {
                if lock_surface.wl_surface() == &root_surface {
                    if matches!(self.awm.lock_state, LockState::WaitingForSurfaces { .. }) {
                        self.awm.maybe_continue_to_locking();
                    } else {
                        self.awm.queue_redraw(&output.clone());
                    }
    ...
    fn destroyed(&mut self, surface: &WlSurface) {
        ...
        if let Some(root) = self.awm.root_surface.get(surface) {
            if let Some((mapped, output)) = self.awm.layout.find_window_and_output(root) {
                let window = mapped.window.clone();
                let output = output.cloned();
        ...
        if let Some(hook) = self.awm.dmabuf_pre_commit_hook.remove(surface) {
...
impl State {
    pub fn add_default_dmabuf_pre_commit_hook(&mut self, surface: &WlSurface) {
        ...
        let s = surface.clone();
        if let Some(prev) = self.awm.dmabuf_pre_commit_hook.insert(s, hook) {
