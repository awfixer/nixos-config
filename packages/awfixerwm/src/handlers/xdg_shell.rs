...
use crate::awm::{CastTarget, PopupGrabState, State};
...
impl XdgShellHandler for State {
    fn xdg_shell_state(&mut self) -> &mut XdgShellState {
        &mut self.awm.xdg_shell_state
    }

    fn new_toplevel(&mut self, surface: ToplevelSurface) {
        let wl_surface = surface.wl_surface().clone();
        let unmapped = Unmapped::new(Window::new_wayland_window(surface));
        let existing = self.awm.unmapped_windows.insert(wl_surface, unmapped);
    ...
    fn new_popup(&mut self, surface: PopupSurface, _positioner: PositionerState) {
        ...
        if let Err(err) = self.awm.popups.track_popup(popup) {
    ...
    fn move_request(&mut self, surface: ToplevelSurface, _seat: WlSeat, serial: Serial) {
        ...
        self.awm.seat.tablet_seat().with_tools(|tools| {
        ...
        let Some((mapped, output)) = self.awm.layout.find_window_and_output(wl_surface) else {
            return;
        };
        ...
        match &start_data {
            ...
            AnyStartData::Touch(_) => {
                let touch = self.awm.seat.get_touch().unwrap();
                if let Some(grab) = MoveGrab::new(self, start_data, window.clone(), true, None) {
    ...
    fn resize_request(
        ...
        let Some((mapped, _)) = self.awm.layout.find_window_and_output(wl_surface) else {
            return;
        };
    ...
    fn grab(&mut self, surface: PopupSurface, _seat: WlSeat, serial: Serial) {
        ...
        if self.awm.exit_confirm_dialog.is_open() {
            ...
            if Some(&root) != self.awm.lock_surface_focus().as_ref() {
            ...
            if let Some(layer) = layers.layer_for_surface(&root, WindowSurfaceType::TOPLEVEL) {
                ...
                if let Some(mapped) = self.awm.mapped_layer_surfaces.get(layer) {
                ...
                if layers.layers_on(Layer::Overlay).any(|l| {
                    (l.cached_state().keyboard_interactivity
                        ...
                        || Some(l) == self.awm.layer_shell_on_demand_focus.as_ref())
                        && self.awm.mapped_layer_surfaces.contains_key(l)
                }) {
                ...
                let layout_focus = self.awm.layout.focus();
        ...
        let can_receive_keyboard_focus = !self.awm.seat.input_method().keyboard_grabbed()
            && self
                .awm
                .layout
                .active_output()
        ...
        self.awm.popup_grab = Some(PopupGrabState {
            root,
            grab,
    ...
    fn maximize_request(&mut self, toplevel: ToplevelSurface) {
            ...
            .find_window_and_output_mut(toplevel.wl_surface())
        {
            ...
            let window = mapped.window.clone();
            self.awm.layout.set_maximized(&window, true);
        } else if let Some(unmapped) = self.awm.unmapped_windows.get_mut(toplevel.wl_surface()) {
            match &mut unmapped.state {
                ...
                InitialConfigureState::Configured {
                    ...
                    let mon = output
                        .as_ref()
                        .and_then(|o| self.awm.layout.monitor_for_output(o))
                        .map(|mon| (mon, false))
                        // If not, check if we have a parent with a monitor.
                        .or_else(|| {
                            toplevel
                                .parent()
                                .and_then(|parent| self.awm.layout.find_window_and_output(&parent))
                    ...
                    *output = mon
                    ...
                    let ws = mon
                        ...
                        .or_else(|| self.awm.layout.active_workspace());
    ...
    fn unmaximize_request(&mut self, toplevel: ToplevelSurface) {
            ...
            .find_window_and_output_mut(toplevel.wl_surface())
        {
            ...
            let window = mapped.window.clone();
            self.awm.layout.set_maximized(&window, false);
        } else if let Some(unmapped) = self.awm.unmapped_windows.get_mut(toplevel.wl_surface()) {
            match &mut unmapped.state {
                InitialConfigureState::NotConfigured {
                    wants_maximized, ..
                ...
                InitialConfigureState::Configured {
                    ...
                    let mon = workspace_name
                        .as_deref()
                        .and_then(|name| self.awm.layout.monitor_for_workspace(name))
                        .map(|mon| (mon, false));
                        ...
                        output
                            ...
                            .or_else(|| {
                                toplevel
                                    .parent()
                                    .and_then(|parent| {
                                        self.awm.layout.find_window_and_output(&parent)
                                    })
                                    .and_then(|(_win, output)| output)
                                    .and_then(|o| self.awm.layout.monitor_for_output(o))
                                    .map(|mon| (mon, true))
    ...
    fn fullscreen_request(
        ...
        let requested_output = wl_output.and_then(|o| self.awm.output_from_resource(&o));
            ...
            .find_window_and_output_mut(toplevel.wl_surface())
        {
            ...
            self.awm.layout.set_fullscreen(&window, true);
        } else if let Some(unmapped) = self.awm.unmapped_windows.get_mut(toplevel.wl_surface()) {
            match &mut unmapped.state {
                InitialConfigureState::NotConfigured {
                    wants_fullscreen, ..
    ...
    fn unfullscreen_request(&mut self, toplevel: ToplevelSurface) {
            ...
            .find_window_and_output_mut(toplevel.wl_surface())
        {
            ...
            let window = mapped.window.clone();
            self.awm.layout.set_fullscreen(&window, false);
        } else if let Some(unmapped) = self.awm.unmapped_windows.get_mut(toplevel.wl_surface()) {
            match &mut unmapped.state {
                InitialConfigureState::NotConfigured {
                    wants_fullscreen, ..
    ...
    fn toplevel_destroyed(&mut self, surface: ToplevelSurface) {
        ...
        let active_window = self.awm.layout.focus().map(|m| &m.window);
        let was_active = active_window == Some(&window);

        self.awm.window_mru_ui.remove_window(id);
        self.awm.layout.remove_window(&window, transaction.clone());
        ...
        if !transaction.is_last() {
            transaction.register_deadline_timer(&self.awm.event_loop);
        }
    ...
    fn popup_destroyed(&mut self, surface: PopupSurface) {
        if let Some(output) = self.output_for_popup(&PopupKind::Xdg(surface)) {
            self.awm.queue_redraw(&output.clone());
        }
    }
    ...
    fn parent_changed(&mut self, toplevel: ToplevelSurface) {
        ...
        if let Some((mapped, output)) = self.awm.layout.find_window_and_output_mut(&parent) {
            let output = output.cloned();
            let window = mapped.window.clone();
            if self.awm.layout.descendants_added(&window) {
                if let Some(output) = output {
                    self.awm.queue_redraw(&output);
                }
            }
...
impl XdgForeignHandler for State {
    fn xdg_foreign_state(&mut self) -> &mut XdgForeignState {
        &mut self.awm.xdg_foreign_state
    }
}
...
impl State {
    pub fn send_initial_configure(&mut self, toplevel: &ToplevelSurface) {
        ...
        let Some(unmapped) = self.awm.unmapped_windows.get_mut(toplevel.wl_surface()) else {
    ...
    pub fn queue_initial_configure(&self, toplevel: ToplevelSurface) {
        ...
        // initial commit.
        self.awm.event_loop.insert_idle(move |state| {
            if !toplevel.alive() {
                return;
            }

            if let Some(unmapped) = state.awm.unmapped_windows.get(toplevel.wl_surface()) {
    ...
    pub fn popups_handle_commit(&mut self, surface: &WlSurface) {
        self.awm.popups.commit(surface);

        if let Some(popup) = self.awm.popups.find_popup(surface) {
            match popup {
    ...
    pub fn output_for_popup(&self, popup: &PopupKind) -> Option<&Output> {
        ...
        self.awm.output_for_root(&root)
    }

    pub fn unconstrain_popup(&self, popup: &PopupKind) {
        let _span = tracy_client::span!("AWM::unconstrain_popup");
        ...
        if let Some((mapped, _)) = self.awm.layout.find_window_and_output(&root) {
            self.unconstrain_window_popup(popup, &mapped.window);
        } else if let Some((layer_surface, output)) = self.awm.layout.outputs().find_map(|o| {
    ...
    fn unconstrain_window_popup(&self, popup: &PopupKind, window: &Window) {
        ...
        let mut target = self.awm.layout.popup_target_rect(window);
    ...
    pub fn update_reactive_popups(&self, window: &Window) {
        let _span = tracy_client::span!("AWM::update_reactive_popups");
    ...
    pub fn update_window_rules(&mut self, toplevel: &ToplevelSurface) {
        ...
        if let Some(unmapped) = self.awm.unmapped_windows.get_mut(toplevel.wl_surface()) {
            ...
            .find_window_and_output_mut(toplevel.wl_surface())
        {
            if mapped.recompute_window_rules(window_rules, self.awm.is_at_startup) {
                drop(config);
                let output = output.cloned();
                let window = mapped.window.clone();
                self.awm.layout.update_window(&window, None);
...
pub fn add_mapped_toplevel_pre_commit_hook(toplevel: &ToplevelSurface) -> HookId {
    add_pre_commit_hook::<State, _>(toplevel.wl_surface(), move |state, _dh, surface| {
        ...
        let Some((mapped, output)) = state.awm.layout.find_window_and_output_mut(surface) else {
        ...
        if let Some(serial) = commit_serial {
            ...
            if let Some(transaction) = mapped.take_pending_transaction(serial) {
                ...
                let disable = state.awm.config.borrow().debug.disable_transactions;
                if !transaction.is_completed() && !disable {
                    ...
                    transaction.register_deadline_timer(&state.awm.event_loop);
                    ...
                    if !is_last {
                        ...
                        if let Some(client) = surface.client() {
                            transaction.add_notification(
                                state.awm.blocker_cleared_tx.clone(),
                                client.clone(),
                            );
            ...
            if let Some(client) = surface.client() {
                let res = state
                    ...
                    .insert_source(source, move |_, _, state| {
                        ...
                        let display_handle = state.awm.display_handle.clone();
                        state
        ...
        if got_unmapped {
            ...
            state.awm.layout.clear_unmap_snapshot(&window);
        }
    })
}
