...
use crate::awm::{DndIcon, NewClient, State};
...
impl SeatHandler for State {
    ...
    fn cursor_image(&mut self, _seat: &Seat<Self>, mut image: CursorImageStatus) {
        ...
        self.awm.cursor_manager.set_cursor_image(image);
        // FIXME: more granular
        self.awm.queue_redraw_all();
    }

    fn focus_changed(&mut self, seat: &Seat<Self>, focused: Option<&WlSurface>) {
        let dh = &self.awm.display_handle;
...
impl PointerConstraintsHandler for State {
    ...
    fn cursor_position_hint(
        ...
        let Some((ref surface_under_pointer, origin)) = self.awm.pointer_contents.surface else {
            return;
        };
        ...
        let target = self
            ...
            .and_then(|output| self.awm.global_space.output_geometry(output))
        ...
        self.awm.pointer_constraint_position_hint = Some(target);
    }
    ...
    fn remove_constraint(
        ...
        let Some(target) = self.awm.pointer_constraint_position_hint.take() else {
...
impl KeyboardShortcutsInhibitHandler for State {
    fn keyboard_shortcuts_inhibit_state(&mut self) -> &mut KeyboardShortcutsInhibitState {
        &mut self.awm.keyboard_shortcuts_inhibit_state
    }
...
impl DataDeviceHandler for State {
    fn data_device_state(&mut self) -> &mut DataDeviceState {
        &mut self.awm.data_device_state
    }
}
...
impl WaylandDndGrabHandler for State {
    fn dnd_requested<S: dnd::Source>(
        ...
        self.awm.dnd_icon = icon.map(|surface| DndIcon {
            surface,
        ...
        match type_ {
            dnd::GrabType::Pointer => {
                ...
                let grab =
                    DnDGrab::new_pointer(&self.awm.display_handle, start_data, source, seat);
                pointer.set_grab(self, grab, serial, Focus::Keep);
            }
            dnd::GrabType::Touch => {
                let touch = seat.get_touch().unwrap();
                let start_data = touch.grab_start_data().unwrap();
                let grab = DnDGrab::new_touch(&self.awm.display_handle, start_data, source, seat);
...
impl DndGrabHandler for State {
    fn dropped(
        ...
        if let Some(target) = validated.then_some(target).flatten() {
            let root = self.awm.find_root_shell_surface(target);
...
impl SessionLockHandler for State {
    fn lock_state(&mut self) -> &mut SessionLockManagerState {
        &mut self.awm.session_lock_state
    }

    fn lock(&mut self, confirmation: SessionLocker) {
        self.awm.lock(confirmation);
    }
    ...
    fn new_surface(&mut self, surface: LockSurface, output: WlOutput) {
        let Some(output) = self.awm.output_from_resource(&output) else {
            warn!("no Output matching WlOutput");
            return;
        };

        configure_lock_surface(&surface, &output);
        self.awm.new_lock_surface(surface, &output);
    }
}
...
impl IdleNotifierHandler for State {
    fn idle_notifier_state(&mut self) -> &mut IdleNotifierState<Self> {
        &mut self.awm.idle_notifier_state
    }
}

impl IdleInhibitHandler for State {
    fn inhibit(&mut self, surface: WlSurface) {
        self.awm.idle_inhibiting_surfaces.insert(surface);
    }

    fn uninhibit(&mut self, surface: WlSurface) {
        self.awm.idle_inhibiting_surfaces.remove(&surface);
    }
}

impl ForeignToplevelHandler for State {
    fn foreign_toplevel_manager_state(&mut self) -> &mut ForeignToplevelManagerState {
        &mut self.awm.foreign_toplevel_state
    }

    fn activate(&mut self, wl_surface: WlSurface) {
        if let Some((mapped, _)) = self.awm.layout.find_window_and_output(&wl_surface) {
            let window = mapped.window.clone();
            self.awm.layout.activate_window(&window);
            self.awm.layer_shell_on_demand_focus = None;
            self.awm.queue_redraw_all();
    ...
    fn set_fullscreen(&mut self, wl_surface: WlSurface, wl_output: Option<WlOutput>) {
        if let Some((mapped, current_output)) = self.awm.layout.find_window_and_output(&wl_surface)
        {
            let window = mapped.window.clone();

            if let Some(requested_output) =
                wl_output.and_then(|o| self.awm.output_from_resource(&o))
            {
...
impl ExtWorkspaceHandler for State {
    fn ext_workspace_manager_state(&mut self) -> &mut ExtWorkspaceManagerState {
        &mut self.awm.ext_workspace_state
    }

    fn activate_workspace(&mut self, id: WorkspaceId) {
        let reference = awm_config::WorkspaceReference::Id(id.get());
        if let Some((mut output, index)) = self.awm.find_output_and_workspace_index(reference) {
    ...
    fn assign_workspace(&mut self, ws_id: WorkspaceId, output: Output) {
        let reference = awm_config::WorkspaceReference::Id(ws_id.get());
        if let Some((old_output, old_idx)) = self.awm.find_output_and_workspace_index(reference) {
            self.awm
                .layout
...
impl ScreencopyHandler for State {
    fn frame(&mut self, manager: &ZwlrScreencopyManagerV1, screencopy: Screencopy) {
        ...
        if !self.awm.output_exists(screencopy.output()) {
        ...
        if screencopy.with_damage() {
            self.awm.screencopy_state.push(manager, screencopy);
        } else {
...
impl GammaControlHandler for State {
    fn gamma_control_manager_state(&mut self) -> &mut GammaControlManagerState {
        &mut self.awm.gamma_control_manager_state
    }
        ...
        if token_data.timestamp.elapsed() < XDG_ACTIVATION_TOKEN_TIMEOUT {
            if let Some((mapped, _)) = self.awm.layout.find_window_and_output_mut(&surface) {
                ...
                match mapped.rules().on_xdg_activate {
                    Some(awm_config::OnXdgActivate::Ignore) => {}
                    Some(awm_config::OnXdgActivate::SetUrgent) => {
                        mapped.set_urgent(true);
                        self.awm.queue_redraw_all();
                    }
                    Some(awm_config::OnXdgActivate::Focus) => {
                        self.awm.layout.activate_window(&window);
                        self.awm.layer_shell_on_demand_focus = None;
                        self.awm.queue_redraw_all();
                    }
            ...
            } else if let Some(unmapped) = self.awm.unmapped_windows.get_mut(&surface) {
                unmapped.activation_token_data = Some(token_data);
            }
        }

        self.awm.activation_state.remove_token(&token);
    }
}
...
impl OutputManagementHandler for State {
    fn output_management_state(&mut self) -> &mut OutputManagementManagerState {
        &mut self.awm.output_management_state
    }

    fn apply_output_config(&mut self, config: awm_config::Outputs) {
        self.awm.config.borrow_mut().outputs = config;
