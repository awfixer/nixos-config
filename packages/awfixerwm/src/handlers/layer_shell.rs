...
impl WlrLayerShellHandler for State {
    fn shell_state(&mut self) -> &mut WlrLayerShellState {
        &mut self.awm.layer_shell_state
    }
    ...
    fn new_layer_surface(
        ...
        let output = if let Some(wl_output) = &wl_output {
            self.awm.output_from_resource(wl_output)
        } else {
            self.awm.layout.active_output().cloned()
        };
        ...
        let is_new = self.awm.unmapped_layer_surfaces.insert(wl_surface);
        assert!(is_new);
    ...
    fn layer_destroyed(&mut self, surface: WlrLayerSurface) {
        ...
        self.awm.unmapped_layer_surfaces.remove(wl_surface);

        let output = if let Some((output, mut map, layer)) =
            self.awm.layout.outputs().find_map(|o| {
            ...
            self.awm.mapped_layer_surfaces.remove(&layer);
            Some(output)
        } else {
            None
        };
...
impl State {
    pub fn layer_shell_handle_commit(&mut self, surface: &WlSurface) -> bool {
        ...
        if is_mapped(surface) {
            let was_unmapped = self.awm.unmapped_layer_surfaces.remove(surface);
            ...
            if was_unmapped {
                ...
                let rules = ResolvedLayerRules::compute(rules, layer, self.awm.is_at_startup);
                ...
                if let Some(mapped) = self.awm.mapped_layer_surfaces.get_mut(layer) {
                    // Check if the layer changed.
                    if mapped.take_recompute_rules_on_commit() {
                        let config = self.awm.config.borrow();
                        if mapped
                            .recompute_layer_rules(&config.layer_rules, self.awm.is_at_startup)
                        {
            ...
            //
            // https://github.com/awfixerwm/awm/issues/641
            ...
            if was_unmapped && on_demand {
                ...
                // little annoying.
                self.awm.layer_shell_on_demand_focus = Some(layer.clone());
            }
        } else {
            // The surface is unmapped.
            if self.awm.mapped_layer_surfaces.remove(layer).is_some() {
                // A mapped surface got unmapped via a null commit. Now it needs to do a new
                // initial commit again.
                self.awm.unmapped_layer_surfaces.insert(surface.clone());
            } else {
...
fn add_mapped_layer_pre_commit_hook(layer: &LayerSurface) -> HookId {
    add_pre_commit_hook::<State, _>(layer.wl_surface(), move |state, _dh, surface| {
        ...
        if layer_changed {
            for mapped in state.awm.mapped_layer_surfaces.values_mut() {
