...
use awm_config::OutputName;
use awm_ipc::state::{EventStreamState, EventStreamStatePart as _};
use awm_ipc::{
...
impl IpcServer {
    pub fn start(
        ...
        let socket_path = if let Some(wayland_socket_name) = wayland_socket_name {
            ...
            let socket_name = format!("awm.{wayland_socket_name}.{}.sock", process::id());
...
fn on_new_ipc_client(state: &mut State, stream: UnixStream) {
    ...
    let stream = match state.awm.event_loop.adapt_io(stream) {
        Ok(stream) => stream,
        Err(err) => {
    ...
    let ipc_server = state.awm.ipc_server.as_ref().unwrap();

    let ctx = ClientCtx {
        event_loop: state.awm.event_loop.clone(),
        scheduler: state.awm.scheduler.clone(),
    ...
    if let Err(err) = state.awm.scheduler.schedule(future) {
    ...
    let response = match request {
        ...
        Request::Layers => {
            ...
            ctx.event_loop.insert_idle(move |state| {
                ...
                for output in state.awm.global_space.outputs() {
                    let name = output.name();
                    for surface in layer_map_for_output(output).layers() {
                        let layer = match surface.layer() {
                            Layer::Background => awm_ipc::Layer::Background,
                            Layer::Bottom => awm_ipc::Layer::Bottom,
                            Layer::Top => awm_ipc::Layer::Top,
                            Layer::Overlay => awm_ipc::Layer::Overlay,
                        };
                        let keyboard_interactivity =
                            match surface.cached_state().keyboard_interactivity {
                                KeyboardInteractivity::None => {
                                    awm_ipc::LayerSurfaceKeyboardInteractivity::None
                                }
                                KeyboardInteractivity::Exclusive => {
                                    awm_ipc::LayerSurfaceKeyboardInteractivity::Exclusive
                                }
                                KeyboardInteractivity::OnDemand => {
                                    awm_ipc::LayerSurfaceKeyboardInteractivity::OnDemand
                                }
                            };

                        layers.push(awm_ipc::LayerSurface {
        ...
        Request::PickWindow => {
            let (tx, rx) = async_channel::bounded(1);
            ctx.event_loop.insert_idle(move |state| {
                let pointer = state.awm.seat.get_pointer().unwrap();
...
fn validate_action(action: &Action) -> Result<(), String> {
    ...
    | Action::LoadConfigFile { path } = action
    {
        if let Some(path) = path {
            // Relative paths are resolved against the awm compositor's working directory, which
...
fn make_ipc_window(
    ...
    with_toplevel_role(mapped.toplevel(), |role| awm_ipc::Window {
        id: mapped.id().get(),
...
impl State {
    pub fn ipc_keyboard_layouts_changed(&mut self) {
        ...
        let Some(server) = &self.awm.ipc_server else {
            return;
        };
    ...
    pub fn ipc_refresh_overview(&mut self) {
        ...
        let is_open = self.awm.layout.is_overview_open();
    ...
    pub fn ipc_refresh_casts(&mut self) {
        ...
        #[cfg(feature = "xdp-gnome-screencast")]
        {
            ...
            for pending in &self.awm.casting.pending_dynamic_casts {
                ...
                if !state.casts.contains_key(&stream_id) {
                    let cast = awm_ipc::Cast {
                        session_id: pending.session_id.get(),
                        stream_id,
                        kind: awm_ipc::CastKind::PipeWire,
                        target: awm_ipc::CastTarget::Nothing {},
            ...
            // Check active casts.
            for cast in &self.awm.casting.casts {
        ...
        self.awm.screencopy_state.clear_expired_casts();

        for queue in self.awm.screencopy_state.queues() {
            if let Some(cast_info) = queue.cast() {
                ...
                if state.casts.get(&stream_id).is_none_or(|existing| {
                    ...
                    match &existing.target {
                        awm_ipc::CastTarget::Output { name } => *name != cast_info.output_name,
                        _ => true,
                    }
                }) {
                    let cast = awm_ipc::Cast {
                        ...
                        stream_id,
                        kind: awm_ipc::CastKind::WlrScreencopy,
                        target: awm_ipc::CastTarget::Output {
