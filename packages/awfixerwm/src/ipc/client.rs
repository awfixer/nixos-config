...
pub fn handle_msg(mut msg: Msg, json: bool) -> anyhow::Result<()> {
    // For actions taking paths, prepend the awm CLI's working directory.
    if let Msg::Action {
        action:
    ...
    let mut socket = Socket::connect().context("error connecting to the awm socket")?;

    let result = socket.send(request);

    // For errors that can be caused by a version mismatch between the running awm instance and
    // the awm msg CLI, we will try to fetch and compare the versions.
    let check_compositor_version = match &result {
        ...
        // Error returned from awm.
        Ok(Err(_)) => true,
        _ => false,
    };

    let compositor_version = if check_compositor_version && !matches!(msg, Msg::Version) {
        // Reconnect to support older awm versions with one request per connection.
        Socket::connect()
    ...
    match compositor_version {
        Some(Ok(Response::Version(compositor_version))) => {
            ...
            if cli_version != compositor_version {
                eprintln!("Running awm compositor has a different version from the awm CLI:");
                eprintln!("Compositor version: {compositor_version}");
                eprintln!("CLI version:        {cli_version}");
                eprintln!("Did you forget to restart awm after an update?");
                eprintln!();
            }
        }
        Some(_) => {
            eprintln!("Unable to get the running awm compositor version.");
            eprintln!("Did you forget to restart awm after an update?");
            eprintln!();
        }
        None => {
    ...
    let reply = result.context("error communicating with awm")?;
    let response = reply.map_err(|err_msg| anyhow!(err_msg).context("awm returned an error"))?;
    ...
    match msg {
        ...
        Msg::Layers => {
            ...
            let print = |surface: &awm_ipc::LayerSurface| {
                println!("    Surface:");
                println!("      Namespace: \"{}\"", surface.namespace);

                let interactivity = match surface.keyboard_interactivity {
                    awm_ipc::LayerSurfaceKeyboardInteractivity::None => "none",
                    awm_ipc::LayerSurfaceKeyboardInteractivity::Exclusive => "exclusive",
                    awm_ipc::LayerSurfaceKeyboardInteractivity::OnDemand => "on-demand",
                };
                println!("      Keyboard interactivity: {interactivity}");
            };

            let print_layer = |iter: &mut Peekable<slice::Iter<awm_ipc::LayerSurface>>,
                               output: &str,
                               layer| {
                let mut empty = true;
            ...
            while let Some(surface) = iter.peek() {
                ...
                print_layer(&mut iter, output, awm_ipc::Layer::Background);

                print!("  Bottom layer:");
                print_layer(&mut iter, output, awm_ipc::Layer::Bottom);

                print!("  Top layer:");
                print_layer(&mut iter, output, awm_ipc::Layer::Top);

                print!("  Overlay layer:");
                print_layer(&mut iter, output, awm_ipc::Layer::Overlay);
            }
        }
        Msg::FocusedOutput => {
        ...
        Msg::EventStream => {
            ...
            loop {
                let event = read_event().context("error reading event from awm")?;
