...
impl Thumbnail {
    fn from_mapped(mapped: &Mapped, clock: Clock, config: awm_config::MruPreviews) -> Self {
        let app_id = with_toplevel_role(mapped.toplevel(), |role| role.app_id.clone());

        let background = FocusRing::new(awm_config::FocusRing {
            off: false,
            width: 0.,
            active_gradient: None,
            ..Default::default()
        });
        let border = FocusRing::new(awm_config::FocusRing {
            off: false,
            active_gradient: None,
            ..Default::default()
        });
    ...
    fn animate_move_from_with_config(&mut self, from: f64, config: awm_config::Animation) {
    ...
    fn animate_open_with_config(&mut self, config: awm_config::Animation) {
    ...
    fn render<R: AWMRenderer>(
        &self,
        mut ctx: RenderCtx<R>,
        config: &awm_config::RecentWindows,
        mapped: &Mapped,
...
impl WindowMru {
    pub fn new(awm: &AWM) -> Self {
        let Some(output) = awm.layout.active_output() else {
            return Self {
                thumbnails: Vec::new(),
                current_id: None,
                scope: MruScope::All,
        ...
        let config = awm.config.borrow().recent_windows.previews;
        let mut thumbnails = Vec::new();
        for (mon, ws_idx, ws) in awm.layout.workspaces() {
            ...
            for mapped in ws.windows() {
                let mut thumbnail = Thumbnail::from_mapped(mapped, awm.clock.clone(), config);
...
impl WindowMruUi {
    ...
    pub fn render_output<R: AWMRenderer>(
        &self,
        awm: &AWM,
        output: &Output,
        mut ctx: RenderCtx<R>,
        ...
        if *output == inner.output && alpha < 1. {
            ...
            inner.render(awm, ctx.r(), &mut |elem| elems.push(elem));
        ...
        if *output == inner.output && alpha == 1. {
            inner.render(awm, ctx, &mut |elem| push(elem));
        }
...
impl Inner {
    ...
    fn render<R: AWMRenderer>(
        ...
        for (thumbnail, geo) in self.thumbnails_in_view_render() {
            let id = thumbnail.id;
            let Some((_, mapped)) = awm.layout.windows().find(|(_, m)| m.id() == id) else {
                error!("window in the MRU must be present in the layout");
                continue;
            };
