...
use awm_ipc::{Action, OutputAction};
...
pub struct Cli {
    /// Path to config file (default: `$XDG_CONFIG_HOME/awm/config.kdl`).
    ///
    /// This can also be set with the `AWM_CONFIG` environment variable. If both are set, the
...
#[derive(Subcommand)]
pub enum Sub {
    /// Communicate with the running awm instance.
    Msg {
        #[command(subcommand)]
        msg: Msg,
...
pub enum Msg {
    ...
    Output {
        /// Output name.
        ///
        /// Run `awm msg outputs` to see the output names.
        #[arg()]
        output: String,
    ...
    EventStream,
    /// Print the version of the running awm instance.
    Version,
    /// Request an error from the running awm instance.
    RequestError,
