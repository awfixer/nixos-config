...
use awm_config::{CornerRadius, Gradient, GradientRelativeTo};
...
impl FocusRing {
    pub fn new(config: awm_config::FocusRing) -> Self {
        Self {
    ...
    pub fn update_config(&mut self, config: awm_config::FocusRing) {
        self.config = config;
    }
    ...
    pub fn config(&self) -> &awm_config::FocusRing {
        &self.config
    }
}
