...
fn set_fullscreen_on_removed_output_does_not_panic() {
    ...
    // Remove the output on the awm side. Its wl_output global is disabled but not yet
    // destroyed, so the client's wl_output resource is still valid and usable.
    let output = f.awm_output(2);
    f.awm().remove_output(&output);

    // Request fullscreen on the now-removed wl_output. awm must not panic.
    let window = f.client(id).window(&surface);
