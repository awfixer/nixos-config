...
impl Fixture {
    ...
    pub fn awm_state(&mut self) -> &mut crate::awm::State {
        &mut self.state.server.state
    }

    pub fn awm(&mut self) -> &mut AWM {
        &mut self.awm_state().awm
    }

    pub fn awm_output(&self, n: u8) -> Output {
        let awm = &self.state.server.state.awm;
        let idx = usize::from(n - 1);
        let output = awm.global_space.outputs().nth(idx).unwrap();
        output.clone()
    }

    pub fn awm_focus_output(&mut self, n: u8) {
        let awm = &mut self.state.server.state.awm;
        let idx = usize::from(n - 1);
        let output = awm.global_space.outputs().nth(idx).unwrap();
        awm.layout.focus_output(output);
    }

    pub fn awm_complete_animations(&mut self) {
        let awm = self.awm();
        awm.clock.set_complete_instantly(true);
        awm.advance_animations();
        awm.clock.set_complete_instantly(false);
    }

    pub fn add_output(&mut self, n: u8, size: (u16, u16)) {
        let state = self.awm_state();
        let awm = &mut state.awm;
        state.backend.headless().add_output(awm, n, size);
    }

    pub fn add_client(&mut self) -> ClientId {
        let (sock1, sock2) = UnixStream::pair().unwrap();
        self.awm().insert_client(NewClient {
            client: sock1,
            restricted: false,
    ...
    ///
    /// I suspect that this is because these configure events are sent from the awm loop callback,
