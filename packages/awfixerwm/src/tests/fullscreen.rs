...
fn windowed_fullscreen() {
    ...
    awm.layout.toggle_windowed_fullscreen(&window_id);
    f.double_roundtrip(id);
...
fn unfullscreen_before_fullscreen_ack_doesnt_prevent_view_offset_save_restore() {
    ...
    assert_snapshot!(f.awm().layout.active_workspace().unwrap().scrolling().view_pos(), @"116");
    ...
    assert_snapshot!(f.awm().layout.active_workspace().unwrap().scrolling().view_pos(), @"-16");
}
