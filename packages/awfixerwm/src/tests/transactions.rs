...
fn format_window_sizes(awm: &AWM) -> String {
    let mut buf = String::new();
    for (_out, mapped) in awm.layout.windows() {
...
fn column_resize_waits_for_both_windows() {
    ...
    f.awm().layout.consume_or_expel_window_left(None);
    f.double_roundtrip(id);
    ...
    assert_snapshot!(format_window_sizes(f.awm()), @r"
    100 × 100
    200 × 200
    ");
