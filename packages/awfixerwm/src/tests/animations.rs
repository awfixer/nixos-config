...
use awm_config::animations::{Curve, EasingParams, Kind};
...
fn format_tiles(awm: &AWM) -> String {
    let mut buf = String::new();
    let ws = awm.layout.active_workspace().unwrap();
...
fn set_time(awm: &mut AWM, time: Duration) {
    ...
    let now = awm.clock.now();
    awm.clock.set_unadjusted(now);
    let _ = awm.clock.now();
    awm.clock.set_unadjusted(Duration::ZERO);
    awm.clock.set_rate(1.0);
    let _ = awm.clock.now();

    // Now, set the desired time at rate 1.0.
    awm.clock.set_unadjusted(time);
    let _ = awm.clock.now();

    // Freeze the clock so that clear() inside the awm loop callback followed by some get()
    // doesn't replace it with the monotonic time.
    awm.clock.set_rate(0.0);
}
...
fn set_up() -> Fixture {
    ...
    f.awm_state().backend.headless().add_renderer().unwrap();
...
fn set_up_two_in_column() -> (Fixture, ClientId, WlSurface, WlSurface) {
    ...
    f.awm().layout.focus_left();
    f.awm().layout.consume_into_column();
    f.double_roundtrip(id);
    ...
    set_time(f.awm(), Duration::ZERO);
    f.awm_complete_animations();
...
fn egl_height_resize_animates_next_y() {
    ...
    assert_snapshot!(format_tiles(f.awm()), @r"
    100 × 100 at x:  0 y:  0
    200 × 200 at x:  0 y:100
    ");

    // Advance the time halfway.
    set_time(f.awm(), Duration::from_millis(500));
    f.awm().advance_animations();
    ...
    set_time(f.awm(), Duration::from_millis(1000));
    f.awm().advance_animations();
...
fn egl_clientside_height_change_doesnt_animate() {
    ...
    // The top window shrinks by itself, without a awm-issued resize.
    let window = f.client(id).window(&surface1);
    window.set_size(100, 50);
    window.commit();
