...
use awm_config::Config;
use awm_ipc::SizeChange;
...
fn unfocus_preserves_current_size() {
    ...
    f.awm().layout.toggle_window_floating(None);
    f.roundtrip(id);
...
fn resize_to_different_size() {
    ...
    f.awm().layout.toggle_window_floating(None);
    f.awm().layout.set_column_width(SizeChange::SetFixed(500));
    f.double_roundtrip(id);
...
fn resize_to_same_size() {
    ...
    f.awm().layout.set_column_width(SizeChange::SetFixed(200));
    ...
    // current server state, and the last size awm requested of the window was 100×100 (even if
...
fn moving_across_workspaces_doesnt_cancel_resize() {
    ...
    f.awm().layout.move_to_workspace_down(true);
...
fn interactive_move_unfullscreen_to_floating_restores_size() {
    ...
    let awm = f.awm();
    let mapped = awm.layout.windows().next().unwrap().1;
    let window = mapped.window.clone();
    awm.layout.set_fullscreen(&window, true);
    f.double_roundtrip(id);
    ...
    let output = f.awm_output(1);
    let awm = f.awm();
    let mapped = awm.layout.windows().next().unwrap().1;
    let window = mapped.window.clone();
    awm.layout
        .interactive_move_begin(window.clone(), &output, Point::default());
    awm.layout.interactive_move_update(
        &window,
        Point::from((1000., 0.)),
        output,
        Point::default(),
...
fn interactive_move_unmaximize_to_floating_restores_size() {
    ...
    let mapped = awm.layout.windows().next().unwrap().1;
    let window = mapped.window.clone();
    awm.layout.set_maximized(&window, true);
    f.double_roundtrip(id);
...
fn resize_during_interactive_move_propagates_to_floating() {
    ...
    f.awm().layout.interactive_move_end(&window_id);
    f.double_roundtrip(id);
...
fn resize_in_steps() {
    ...
    f.awm().layout.set_column_width(SizeChange::SetFixed(600));
...
fn interactive_move_restores_floating_size_when_set_to_floating() {
    ...
    // Change size while we're floating and commit to make awm remember it.
    let window = f.client(id).window(&surface);
    window.set_size(200, 200);
    window.ack_last_and_commit();
    f.double_roundtrip(id);
...
fn unfullscreen_to_same_size_floating() {
    ...
    // Change size to the same as fullscreen, make awm remember it.
    let window = f.client(id).window(&surface);
    window.set_size(1920, 1080);
    window.ack_last_and_commit();
    f.double_roundtrip(id);
...
fn unmaximize_to_same_size_floating() {
    ...
    // Change size to the same as maximized, make awm remember it.
    let window = f.client(id).window(&surface);
    window.set_size(1920, 1080);
    window.ack_last_and_commit();
    f.double_roundtrip(id);
...
fn unfullscreen_to_same_size_windowed_fullscreen_floating() {
    ...
    let mapped = f.awm().layout.windows().next().unwrap().1;
    let window_id = mapped.window.clone();
    ...
    f.awm().layout.toggle_windowed_fullscreen(&window_id);
    f.double_roundtrip(id);
