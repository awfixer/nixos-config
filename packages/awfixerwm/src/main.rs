...
use awm::cli::{Cli, CompletionShell, Sub};
#[cfg(feature = "dbus")]
use awm::dbus;
use awm::ipc::client::handle_msg;
use awm::awm::State;
use awm::utils::spawning::{
...
use awm::utils::{cause_panic, version, watcher, xwayland, IS_SYSTEMD_SERVICE};
use awm_config::{Config, ConfigPath};
use awm_ipc::socket::SOCKET_PATH_ENV;
use sd_notify::NotifyState;
use smithay::reexports::wayland_server::Display;
use tracing_subscriber::EnvFilter;

const DEFAULT_LOG_FILTER: &str = "awm=debug,smithay::backend::renderer::gles=error";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.session {
        ...
        env::set_var("XDG_CURRENT_DESKTOP", "awm");
    ...
    awm::utils::signals::block_early().unwrap();

    // Avoid starting Tracy for the `awm msg` code path since starting/stopping Tracy is a bit
    // slow.
    tracy_client::Client::start();
    ...
    *CHILD_ENV.write().unwrap() = mem::take(&mut config.environment);
    ...
    awm::utils::signals::listen(&event_loop.handle());
    ...
    let socket_name = state.awm.socket_name.as_deref().unwrap();
    ...
    // Set AWM_SOCKET for children.
    if let Some(ipc) = &state.awm.ipc_server {
    ...
    if let Some(satellite) = &state.awm.satellite {
    ...
    if cli.session {
        ...
        #[cfg(feature = "dbus")]
        if !state.awm.config.borrow().input.disable_power_key_handling {
            if let Err(err) = state.awm.inhibit_power_key() {
    ...
    #[cfg(feature = "dbus")]
    if cli.session {
        state.awm.a11y.start();
    }

    if env::var_os("AWM_DISABLE_SYSTEM_MANAGER_NOTIFY").is_none_or(|x| x != "1") {
    ...
    if config_errored {
        state.awm.config_error_notification.show();
        state.ipc_config_loaded(true);
    } else if let Some(path) = config_created_at {
        state.awm.config_error_notification.show_created(path);
    }
...
fn env_config_path() -> Option<PathBuf> {
    env::var_os("AWM_CONFIG")
        .filter(|x| !x.is_empty())
        .map(PathBuf::from)
}

fn default_config_path() -> Option<PathBuf> {
    let Some(dirs) = ProjectDirs::from("", "", "awm") else {
...
fn system_config_path() -> PathBuf {
    PathBuf::from("/etc/awm/config.kdl")
}
