...
use crate::awm::{CastTarget, State};
...
pub struct PipeWire {
    ...
    to_awm: calloop::channel::Sender<PwToAWM>,
}
...
impl PipeWire {
        ...
        let listener = core
            ...
            .error(move |id, seq, res, message| {
                ...
                if id == PW_ID_CORE && res == -32 {
                    if let Err(err) = to_awm_.send(PwToAWM::FatalError) {
                        warn!("error sending FatalError to awm: {err:?}");
                    }
                }
            })
            .register();
    ...
    pub fn start_cast(
        ...
        let to_awm_ = self.to_awm.clone();
        let stop_cast = move || {
            if let Err(err) = to_awm_.send(PwToAWM::StopCast { session_id }) {
                warn!(%session_id, "error sending StopCast to awm: {err:?}");
            }
        };
        let to_awm_ = self.to_awm.clone();
        let redraw = move || {
            if let Err(err) = to_awm_.send(PwToAWM::Redraw { stream_id }) {
                warn!(%stream_id, "error sending Redraw to awm: {err:?}");
            }
        };
            ...
            Some(sync_fd) => {
                ...
                self.event_loop
                    .insert_source(source, move |_, _, state| {
                        for cast in &mut state.awm.casting.casts {
