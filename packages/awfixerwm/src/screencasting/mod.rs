...
use crate::dbus::mutter_screen_cast::{self, CursorMode, ScreenCastToAWM, StreamTargetId};
use crate::awm::{CastTarget, AWM, OutputRenderElements, PointerRenderElements, State};
...
mod pw_utils;
use pw_utils::{Cast, CastSizeChange, CursorData, PipeWire, PwToAWM};
...
pub struct Screencasting {
    ...
    pub pw_to_awm: calloop::channel::Sender<PwToAWM>,
...
impl Screencasting {
    pub fn new(event_loop: &LoopHandle<'static, State>) -> Self {
        let pw_to_awm = {
            let (pw_to_awm, from_pipewire) = calloop::channel::channel();
            event_loop
...
impl State {
    fn prepare_pw_cast(&mut self) -> anyhow::Result<(GbmDevice<DrmDeviceFd>, FormatSet)> {
        ...
        if self.awm.casting.pipewire.is_none() {
            let pw = PipeWire::new(
                self.awm.event_loop.clone(),
                self.awm.casting.pw_to_awm.clone(),
            )
            .context("error initializing PipeWire")?;
            self.awm.casting.pipewire = Some(pw);
        }
    ...
    pub fn on_pw_msg(&mut self, msg: PwToAWM) {
        match msg {
            PwToAWM::StopCast { session_id } => self.awm.stop_cast(session_id),
            PwToAWM::Redraw { stream_id } => self.redraw_cast(stream_id),
            PwToAWM::FatalError => {
                warn!("stopping PipeWire due to fatal error");
                let casting = &mut self.awm.casting;
                if let Some(pw) = casting.pipewire.take() {
                    ...
                    for id in ids {
                        self.awm.stop_cast(id);
                    }
                    self.awm.event_loop.remove(pw.token);
                }
            }
        }
    }
    ...
    fn redraw_cast(&mut self, stream_id: CastStreamId) {
        ...
        let casts = &mut self.awm.casting.casts;
        ...
        let mut casts = mem::take(&mut self.awm.casting.casts);
        let cast = &mut casts[idx];
        let mut stop = false;
        ...
        loop {
            ...
            let Some(output) = self.awm.casting.mapped_cast_output.get(&mapped.window) else {
                break;
            };
            ...
            self.backend.with_primary_renderer(|renderer| {
                ...
                if self.awm.pointer_visibility.is_visible() {
                    if let Some((pointer_pos, win_pos)) =
                        self.awm.pointer_pos_for_window_cast(mapped)
                    {
        ...
        self.awm.casting.casts = casts;

        if stop {
            self.awm.stop_cast(session_id);
        }
    }
    ...
    pub fn set_dynamic_cast_target(&mut self, target: CastTarget) {
        ...
        match &target {
            ...
            CastTarget::Window { id } => {
                ...
                if let Some((_, mapped)) = windows.find(|(_, mapped)| mapped.id().get() == *id) {
                    if let Some(output) = self.awm.casting.mapped_cast_output.get(&mapped.window) {
                        refresh = Some(output.current_mode().unwrap().refresh as u32);
                    }
                }
            }
        ...
        for cast in &mut self.awm.casting.casts {
    ...
    fn start_pending_dynamic_casts(&mut self, target: &CastTarget) {
        let pending = &self.awm.casting.pending_dynamic_casts;
        if pending.is_empty() {
            return;
        }
        ...
        let (size, refresh) = match target {
            ...
            CastTarget::Window { id } => {
                let Some((size, refresh)) = self.awm.cast_params_for_window(*id) else {
                    return;
                };
                (size, refresh)
            }
        ...
        let (gbm, render_formats) = match self.prepare_pw_cast() {
            ...
            Err(err) => {
                ...
                let mut ids = HashSet::new();
                for pending in self.awm.casting.pending_dynamic_casts.drain(..) {
        ...
        let pw = self.awm.casting.pipewire.as_ref().unwrap();
        ...
        for pending in self.awm.casting.pending_dynamic_casts.drain(..) {
            ...
            match res {
                Ok(mut cast) => {
                    ...
                    self.awm.casting.casts.push(cast);
                }
                Err(err) => {
    ...
    pub fn on_screen_cast_msg(&mut self, msg: ScreenCastToAWM) {
        match msg {
            ScreenCastToAWM::StartCast {
                session_id,
                stream_id,
                target,
                cursor_mode,
                ...
                let (target, size, refresh, alpha) = match target {
                    StreamTargetId::Output { name } => {
                        let global_space = &self.awm.global_space;
                    ...
                    StreamTargetId::Window { id }
                        if id == self.awm.casting.dynamic_cast_id_for_portal.get() =>
                    {
                        debug!("delaying dynamic cast until target is set");
                        self.awm.casting.pending_dynamic_casts.push(PendingCast {
                            session_id,
                            stream_id,
                            cursor_mode,
                            signal_ctx,
            ...
            ScreenCastToAWM::StopCast { session_id } => self.awm.stop_cast(session_id),
        }
    }
}
...
impl AWM {
    ...
    pub fn render_for_screen_cast(
        ...
        let _span = tracy_client::span!("AWM::render_for_screen_cast");
    ...
    pub fn render_windows_for_screen_cast(
        ...
        let _span = tracy_client::span!("AWM::render_windows_for_screen_cast");
    ...
    pub fn stop_cast(&mut self, session_id: CastSessionId) {
        let _span = tracy_client::span!("AWM::stop_cast");
    ...
    pub fn stop_casts_for_target(&mut self, target: CastTarget) {
        let _span = tracy_client::span!("AWM::stop_casts_for_target");
