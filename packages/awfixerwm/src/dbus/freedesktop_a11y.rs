...
use crate::awm::{PointContents, State};
...
pub enum A11yManagerToAWM {
    QueryPointer,
}

pub enum AWMToA11yManager {
...
pub struct PointerLocator {
    ...
    iface: Arc<OnceLock<InterfaceRef<Self>>>,
    to_awm: calloop::channel::Sender<A11yManagerToAWM>,
    from_awm: async_channel::Receiver<AWMToA11yManager>,
}
...
impl PointerLocator {
    ...
    async fn query_pointer(
        ...
        if let Err(err) = self.to_awm.send(A11yManagerToAWM::QueryPointer) {
            warn!("error sending message to awm: {err:?}");
            return Err(A11yError::Failed("internal error".to_owned()));
        }

        let rv = match self.from_awm.recv().await {
            Ok(AWMToA11yManager::PointerContents(Some((data, x, y)))) => Ok((data, x, y)),
            Ok(AWMToA11yManager::PointerContents(None)) => Err(A11yError::UnknownToplevel),
            Err(err) => {
...
impl State {
    pub fn a11y_process_key(
        ...
        if self.awm.a11y_manager.is_none() {
            return KbMonBlock::Pass;
        }
        ...
        let Some(manager) = &self.awm.a11y_manager else {
            return KbMonBlock::Pass;
        };
    ...
    pub fn on_a11y_manager_msg(
        &mut self,
        to_a11y: &async_channel::Sender<AWMToA11yManager>,
        msg: A11yManagerToAWM,
    ) {
        let A11yManagerToAWM::QueryPointer = msg;
        ...
        let contents = match &self.awm.pointer_contents {
            PointContents {
            ...
            _ => {
                // The focused client is awm itself.
                Some((PointerAppData::default(), pointer_pos.x, pointer_pos.y))
            }
        };

        let msg = AWMToA11yManager::PointerContents(contents);
