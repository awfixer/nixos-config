...
pub enum Login1ToAWM {
    LidClosedChanged(bool),
}

pub fn start(
    to_awm: calloop::channel::Sender<Login1ToAWM>,
    ...
    let future = async move {
        ...
        if let Err(err) = to_awm.send(Login1ToAWM::LidClosedChanged(lid_closed)) {
            warn!("error sending initial lid state to awm: {err:?}");
            return;
        };
