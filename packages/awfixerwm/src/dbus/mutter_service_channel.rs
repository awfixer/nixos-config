...
use super::Start;
use crate::awm::NewClient;

pub struct ServiceChannel {
    to_awm: calloop::channel::Sender<NewClient>,
}
...
impl ServiceChannel {
    async fn open_wayland_service_connection(
        ...
        if let Err(err) = self.to_awm.send(client) {
...
impl ServiceChannel {
    pub fn new(to_awm: calloop::channel::Sender<NewClient>) -> Self {
        Self { to_awm }
    }
}
