...
pub struct DisplayConfig {
    to_awm: calloop::channel::Sender<HashMap<String, Option<awm_config::Output>>>,
        ...
        for output in self.ipc_outputs.lock().unwrap().values() {
            ...
            if let Some(logical) = output.logical.as_ref() {
                let transform = match logical.transform {
                    awm_ipc::Transform::Normal => 0,
                    awm_ipc::Transform::_90 => 1,
                    awm_ipc::Transform::_180 => 2,
                    awm_ipc::Transform::_270 => 3,
                    awm_ipc::Transform::Flipped => 4,
                    awm_ipc::Transform::Flipped90 => 5,
                    awm_ipc::Transform::Flipped180 => 6,
                    awm_ipc::Transform::Flipped270 => 7,
                };
    ...
    async fn apply_monitors_config(
        ...
        for requested_config in logical_monitor_configs {
            ...
            for (connector, mode, _props) in requested_config.monitors {
                ...
                new_conf.insert(
                    connector.clone(),
                    Some(awm_config::Output {
                        off: false,
                        name: connector,
                        scale: Some(awm_config::FloatOrInt(requested_config.scale)),
                        transform: match requested_config.transform {
                            0 => awm_ipc::Transform::Normal,
                            1 => awm_ipc::Transform::_90,
                            2 => awm_ipc::Transform::_180,
                            3 => awm_ipc::Transform::_270,
                            4 => awm_ipc::Transform::Flipped,
                            5 => awm_ipc::Transform::Flipped90,
                            6 => awm_ipc::Transform::Flipped180,
                            7 => awm_ipc::Transform::Flipped270,
                            x => {
                                return Err(zbus::fdo::Error::Failed(format!(
                                    "Unknown transform {x}",
                                )))
                        ...
                        position: Some(awm_config::Position {
                            x: requested_config.x,
                            y: requested_config.y,
                        }),
                        mode: Some(awm_config::output::Mode {
                            custom: false,
                            mode: awm_ipc::ConfiguredMode::from_str(&mode).map_err(|e| {
        ...
        if let Err(err) = self.to_awm.send(new_conf) {
...
// Adapted from Mutter.
fn make_display_name(output: &awm_ipc::Output, is_laptop_panel: bool) -> String {
    if is_laptop_panel {
