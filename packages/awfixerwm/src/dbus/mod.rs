...
impl DBusServers {
    pub fn start(state: &mut State, is_session_instance: bool) {
        ...
        let awm = &mut state.awm;
        let config = awm.config.borrow();

        let mut dbus = Self::default();

        if is_session_instance {
            let (to_awm, from_service_channel) = calloop::channel::channel();
            let service_channel = ServiceChannel::new(to_awm);
            awm.event_loop
                .insert_source(from_service_channel, move |event, _, state| match event {
                    calloop::channel::Event::Msg(new_client) => {
                        state.awm.insert_client(new_client);
                    }
        ...
        if is_session_instance || config.debug.dbus_interfaces_in_non_session_instances {
            let (to_awm, from_display_config) = calloop::channel::channel();
            let display_config = DisplayConfig::new(to_awm, backend.ipc_outputs());
            awm.event_loop
            ...
            let screen_saver = ScreenSaver::new(awm.is_fdo_idle_inhibited.clone());
            dbus.conn_screen_saver = try_start(screen_saver);

            let (to_awm, from_screenshot) = calloop::channel::channel();
            let (to_screenshot, from_awm) = async_channel::unbounded();
            awm.event_loop
                ...
                .unwrap();
            let screenshot = gnome_shell_screenshot::Screenshot::new(to_awm, from_awm);
            dbus.conn_screen_shot = try_start(screenshot);

            let (to_awm, from_introspect) = calloop::channel::channel();
            let (to_introspect, from_awm) = async_channel::unbounded();
            awm.event_loop
                ...
                .unwrap();
            let introspect = Introspect::new(to_awm, from_awm);
            dbus.conn_introspect = try_start(introspect);

            #[cfg(feature = "xdp-gnome-screencast")]
            {
                let (to_awm, from_screen_cast) = calloop::channel::channel();
                awm.event_loop
                    ...
                    .unwrap();
                let screen_cast = ScreenCast::new(backend.ipc_outputs(), to_awm);
                dbus.conn_screen_cast = try_start(screen_cast);
            }

            let (to_awm, from_a11y) = calloop::channel::channel();
            let (to_a11y, from_awm) = async_channel::unbounded();
            awm.event_loop
                ...
                .unwrap();
            let a11y_manager = freedesktop_a11y::Manager::new(to_awm, from_awm);
            match a11y_manager.start() {
                Ok(conn) => {
                    dbus.conn_a11y_manager = Some(conn);
                    awm.a11y_manager = Some(a11y_manager);
                }
                Err(err) => {
        ...
        let (to_awm, from_login1) = calloop::channel::channel();
        awm.event_loop
            ...
            .unwrap();
        match freedesktop_login1::start(to_awm) {
            Ok(conn) => {
        ...
        let (to_awm, from_locale1) = calloop::channel::channel();
        awm.event_loop
            ...
            .unwrap();
        match freedesktop_locale1::start(to_awm) {
            Ok(conn) => {
