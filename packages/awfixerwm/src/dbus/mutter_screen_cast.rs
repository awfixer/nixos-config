...
pub struct ScreenCast {
    ...
    to_awm: calloop::channel::Sender<ScreenCastToAWM>,
...
impl ScreenCast {
    async fn create_session(
        ...
        let session = Session::new(session_id, self.ipc_outputs.clone(), self.to_awm.clone());
...
impl Session {
    ...
    pub async fn stop(
        ...
        if let Err(err) = self.to_awm.send(ScreenCastToAWM::StopCast {
            session_id: self.id,
        }) {
            warn!("error sending StopCast to awm: {err:?}");
        }
...
impl Drop for Session {
    fn drop(&mut self) {
        let _ = self.to_awm.send(ScreenCastToAWM::StopCast {
            session_id: self.id,
        });
    }
}
...
impl Stream {
    ...
    fn start(&self, ctxt: SignalEmitter<'static>) {
        ...
        let msg = ScreenCastToAWM::StartCast {
        ...
        if let Err(err) = self.to_awm.send(msg) {
            warn!("error sending StartCast to awm: {err:?}");
        }
    }
}
