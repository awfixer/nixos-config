...
use awm_config::{FloatOrInt, OutputName, Vrr};
use awm_ipc::Transform;
...
pub struct OutputManagementManagerState {
    ...
    current_state: HashMap<OutputId, awm_ipc::Output>,
    current_config: awm_config::Outputs,
}
...
enum OutputConfigurationState {
    Ongoing(HashMap<OutputId, awm_config::Output>),
    Finished,
}
...
impl OutputManagementManagerState {
    ...
    pub fn on_config_changed(&mut self, new_config: awm_config::Outputs) {
        self.current_config = new_config;
    }

    pub fn notify_changes(&mut self, new_state: HashMap<OutputId, awm_ipc::Output>) {
                ...
                match new_config.entry(*output) {
                    ...
                    Entry::Vacant(entry) => {
                        ...
                        let mut config = g_state
                            ...
                            .find(&name)
                            .cloned()
                            .unwrap_or_else(|| awm_config::Output {
    ...
    fn request(
        ...
        match request {
            zwlr_output_configuration_head_v1::Request::SetMode { mode } => {
                ...
                new_config.mode = Some(awm_config::output::Mode {
                    custom: false,
                    mode: awm_ipc::ConfiguredMode {
                        width: mode.width,
                        height: mode.height,
            ...
            zwlr_output_configuration_head_v1::Request::SetPosition { x, y } => {
                new_config.position = Some(awm_config::Position { x, y });
            }
