...
pub fn refresh(state: &mut State) {
    ...
    let protocol_state = &mut state.awm.foreign_toplevel_state;

    // Handle closed windows.
    protocol_state.toplevels.retain(|surface, data| {
        if state.awm.layout.find_window_and_output(surface).is_some() {
            return true;
        }
    ...
    state.awm.layout.with_windows(|mapped, output, _, _| {
        ...
        with_toplevel_role_and_current(toplevel, |role, cur| {
            ...
            if state.awm.keyboard_focus.surface() == Some(wl_surface) {
