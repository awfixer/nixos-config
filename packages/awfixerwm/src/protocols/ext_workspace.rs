...
//!
//! This is how we map the protocol concepts to the awm concepts:
//!
...
pub fn refresh(state: &mut State) {
    ...
    let protocol_state = &mut state.awm.ext_workspace_state;
    ...
    for (mon, _, ws) in state.awm.layout.workspaces() {
    ...
    protocol_state.workspace_groups.retain(|output, data| {
        if state.awm.sorted_outputs.contains(output) {
            return true;
        }
    ...
    for (mon, ws_idx, ws) in state.awm.layout.workspaces() {
    ...
    for output in &state.awm.sorted_outputs {
