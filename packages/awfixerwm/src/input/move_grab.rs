...
impl MoveGrab {
    pub fn new(
        ...
        let (output, pos_within_output) = state.awm.output_under(location)?;
    ...
    fn on_ungrab(&mut self, data: &mut State) {
        let layout = &mut data.awm.layout;
        match self.gesture {
        ...
        data.awm.queue_redraw_all();
    }

    fn begin_move(&mut self, data: &mut State) -> bool {
        if !data.awm.layout.interactive_move_begin(
            self.window.clone(),
            &self.start_output,
    ...
    fn on_frame(&mut self, data: &mut State) -> bool {
        ...
        match self.gesture {
            ...
            GestureState::Move => {
                let Some((output, pos_within_output)) = data.awm.output_under(self.last_location)
                else {
                    return true;
                };
                ...
                let ongoing = data.awm.layout.interactive_move_update(
                    &self.window,
                    delta,
                    output,
                    pos_within_output,
            ...
            GestureState::ViewOffset => {
                let res = data.awm.layout.view_offset_gesture_update(
                    -relative_delta.x,
                    timestamp,
                    false,
                );
    ...
    fn on_toggle_floating(&mut self, data: &mut State) -> bool {
        ...
        data.awm.layout.toggle_window_floating(Some(&self.window));
        data.awm.queue_redraw_all();
