...
use awm_ipc::LayoutSwitchTarget;
...
use crate::awm::{CastTarget, PointerVisibility, State};
...
use backend_ext::{AWMInputBackend as InputBackend, AWMInputDevice as _};
...
impl State {
        ...
        I::Device: 'static, // Needed for downcasting.
    {
        ...
        if should_reset_pointer_inactivity_timer(&event) {
            self.awm.reset_pointer_inactivity_timer();
        }

        let hide_hotkey_overlay =
            self.awm.hotkey_overlay.is_open() && should_hide_hotkey_overlay(&event);

        let hide_exit_confirm_dialog =
            self.awm.exit_confirm_dialog.is_open() && should_hide_exit_confirm_dialog(&event);
        ...
        if hide_hotkey_overlay && self.awm.hotkey_overlay.hide() {
            self.awm.queue_redraw_all();
        }

        if hide_exit_confirm_dialog && self.awm.exit_confirm_dialog.hide() {
            self.awm.queue_redraw_all();
        }
    }
    ...
    pub fn process_libinput_event(&mut self, event: &mut InputEvent<LibinputInputBackend>) {
        ...
        match event {
            InputEvent::DeviceAdded { device } => {
                self.awm.devices.insert(device.clone());
                ...
                if device.has_capability(input::DeviceCapability::TabletTool) {
                    match device.size() {
                        Some((w, h)) => {
                            ...
                            self.awm.tablets.insert(device.clone(), data);
                        }
                        None => {
                ...
                if device.has_capability(input::DeviceCapability::Touch) {
                    self.awm.touch.insert(device.clone());
                }

                apply_libinput_settings(&self.awm.config.borrow().input, device);
            }
            InputEvent::DeviceRemoved { device } => {
                self.awm.touch.remove(device);
                self.awm.tablets.remove(device);
                self.awm.devices.remove(device);
            }
            _ => (),
        }
    }
    ...
    fn on_device_added(&mut self, device: impl Device) {
        if device.has_capability(DeviceCapability::TabletTool) {
            let tablet_seat = self.awm.seat.tablet_seat();

            let desc = TabletDescriptor::from(&device);
            tablet_seat.add_wp_tablet(&self.awm.display_handle, &desc);
        }
        if device.has_capability(DeviceCapability::Touch) && self.awm.seat.get_touch().is_none() {
            self.awm.seat.add_touch();
        }
    }
    ...
    fn on_device_removed(&mut self, device: impl Device) {
        ...
        if device.has_capability(DeviceCapability::Touch) && self.awm.touch.is_empty() {
            self.awm.seat.remove_touch();
        }
    }
    ...
    fn global_bounding_rectangle(&self) -> Option<Rectangle<i32, Logical>> {
        self.awm.global_space.outputs().fold(
            None,
        ...
        I::Device: 'static,
    {
        ...
        let device_output = device_output.filter(|output| self.awm.output_exists(output));
        let device_output = device_output.as_ref();
        let mapped_output = device_output.or_else(|| self.awm.output_for_tablet());
        ...
        let map_to_focused_window = self.awm.config.borrow().input.tablet.map_to_focused_window;
        // But only if the keyboard focus is on the layout, so that it doesn't trigger on the lock
        // screen and such.
        let window_target = if map_to_focused_window && self.awm.keyboard_focus.is_layout() {
            let output = mapped_output.or_else(|| self.awm.layout.active_output());
            output.and_then(|output| {
                let monitor = self.awm.layout.monitor_for_output(output)?;
                let mut rect = monitor.active_window_visual_rectangle()?;
                let output_geo = self.awm.global_space.output_geometry(output)?;
        ...
        let (target_geo, keep_ratio, px, transform) = if let Some((rect, output)) = window_target {
            ...
            let geo = self.awm.global_space.output_geometry(output).unwrap();
            (
                geo.to_f64(),
                true,
                1. / output.current_scale().fractional_scale(),
            ...
            let output = self.awm.global_space.outputs().next().unwrap();
        ...
        if keep_ratio {
            ...
            if let Some(device) = (&device as &dyn Any).downcast_ref::<input::Device>() {
                if let Some(data) = self.awm.tablets.get(device) {
    ...
    fn on_keyboard<I: InputBackend>(
        ...
        let mod_key = self.backend.mod_key(&self.awm.config.borrow());
        ...
        if !pressed {
            if let Some(token) = self.awm.bind_repeat_timer.take() {
                self.awm.event_loop.remove(token);
            }
        }
        ...
        let Some(Some(bind)) = self.awm.seat.get_keyboard().unwrap().input(
            self,
            event.key_code(),
            event.state(),
            serial,
            ...
            |this, mods, keysym| {
                ...
                if this.awm.exit_confirm_dialog.is_open() && pressed {
                    if raw == Some(Keysym::Return) {
                        info!("quitting after confirming exit dialog");
                        this.awm.stop_signal.stop();
                    }

                    // Don't send this press to any clients.
                    this.awm.suppressed_keys.insert(key_code);
                    return FilterResult::Intercept(None);
                }
                ...
                if this.awm.window_mru_ui.is_open() && !pressed && modifiers.is_empty() {
                    this.do_action(Action::MruConfirm, false);

                    if this.awm.suppressed_keys.remove(&key_code) {
                        return FilterResult::Intercept(None);
                    } else {
                        return FilterResult::Forward;
                    }
                ...
                if pressed && raw == Some(Keysym::Escape) {
                    ...
                    let pointer = this.awm.seat.get_pointer().unwrap();
                    if pointer
                ...
                if let Some(Keysym::space) = raw {
                    this.awm.screenshot_ui.set_space_down(pressed);
                }

                let res = {
                    let config = this.awm.config.borrow();
                    let bindings =
                        make_binds_iter(&config, &mut this.awm.window_mru_ui, modifiers);

                    should_intercept_key(
                        &mut this.awm.suppressed_keys,
                        bindings,
                        mod_key,
                        key_code,
                        modified,
                        ...
                        pressed,
                        *mods,
                        &this.awm.screenshot_ui,
                        this.awm.config.borrow().input.disable_power_key_handling,
                ...
                if matches!(res, FilterResult::Forward) {
                    ...
                    if this.awm.keyboard_focus.is_overview() && pressed {
                    ...
                    this.awm.mru_apply_keyboard_commit();
                }
    ...
    fn start_key_repeat(&mut self, bind: Bind) {
        ...
        let config = self.awm.config.borrow();
        ...
        self.awm.bind_repeat_timer = Some(token);
    }

    fn hide_cursor_if_needed(&mut self) {
        // If the pointer is already invisible, don't reset it back to Hidden causing one frame
        // of hover.
        if !self.awm.pointer_visibility.is_visible() {
            return;
        }

        if !self.awm.config.borrow().cursor.hide_when_typing {
            return;
        }

        // awm keeps this set only while actively using a tablet, which means the cursor position
        // is likely to change almost immediately, causing pointer_visibility to just flicker back
        // and forth.
        if self.awm.tablet_cursor_location.is_some() {
            return;
        }

        self.awm.pointer_visibility = PointerVisibility::Hidden;
        self.awm.queue_redraw_all();
    }
    ...
    pub fn handle_bind(&mut self, bind: Bind) {
        ...
        if self.awm.is_locked() && !(bind.allow_when_locked || allowed_when_locked(&bind.action)) {
            return;
        }

        match self.awm.bind_cooldown_timers.entry(bind.key) {
            ...
            Entry::Vacant(entry) => {
                ...
                let token = self
                    ...
                    .insert_source(timer, move |_, _, state| {
                        if state.awm.bind_cooldown_timers.remove(&bind.key).is_none() {
    ...
    pub fn do_action(&mut self, action: Action, allow_when_locked: bool) {
        if self.awm.is_locked() && !(allow_when_locked || allowed_when_locked(&action)) {
            return;
        }

        if let Some(touch) = self.awm.seat.get_touch() {
            touch.cancel(self);
        }

        match action {
            Action::Quit(skip_confirmation) => {
                if !skip_confirmation && self.awm.exit_confirm_dialog.show() {
                    self.awm.queue_redraw_all();
                    return;
                }

                info!("quitting as requested");
                self.awm.stop_signal.stop()
            }
            Action::ChangeVt(vt) => {
                self.backend.change_vt(vt);
                // Changing VT may not deliver the key releases, so clear the state.
                self.awm.suppressed_keys.clear();
            }
            Action::Suspend => {
            ...
            Action::PowerOffMonitors => {
                self.awm.deactivate_monitors(&mut self.backend);
            }
            ...
            Action::DebugToggleOpaqueRegions => {
                self.awm.debug_draw_opaque_regions = !self.awm.debug_draw_opaque_regions;
                self.awm.queue_redraw_all();
            }
            Action::DebugToggleDamage => {
                self.awm.debug_toggle_damage();
            }
            Action::Spawn(command) => {
                let (token, _) = self.awm.activation_state.create_external_token(None);
            ...
            Action::DoScreenTransition(delay_ms) => {
                self.backend.with_primary_renderer(|renderer| {
                    self.awm.do_screen_transition(renderer, delay_ms);
                });
            }
            Action::ScreenshotScreen(write_to_disk, show_pointer, path) => {
                let active = self.awm.layout.active_output().cloned();
                if let Some(active) = active {
                    self.backend.with_primary_renderer(|renderer| {
                        if let Err(err) = self.awm.screenshot(
                            renderer,
                            &active,
                            write_to_disk,
                            show_pointer,
            ...
            Action::CancelScreenshot => {
                if !self.awm.screenshot_ui.is_open() {
                    return;
                }

                self.awm.screenshot_ui.close();
                self.awm
                    .cursor_manager
            ...
            Action::ScreenshotTogglePointer => {
                self.awm.screenshot_ui.toggle_pointer();
                self.awm.queue_redraw_all();
            }
            ...
            Action::ScreenshotWindow(write_to_disk, show_pointer, path) => {
                let focus = self.awm.layout.focus_with_output();
                if let Some((mapped, output)) = focus {
                    self.backend.with_primary_renderer(|renderer| {
                        if let Err(err) = self.awm.screenshot_window(
                            renderer,
                            output,
                            mapped,
                            write_to_disk,
            ...
            Action::ScreenshotWindowById {
                ...
                let mut windows = self.awm.layout.windows();
            ...
            Action::ToggleKeyboardShortcutsInhibit => {
                if let Some(inhibitor) = self.awm.keyboard_focus.surface().and_then(|surface| {
                    self.awm
                        .keyboard_shortcuts_inhibiting_surfaces
                        .get(surface)
                }) {
            ...
            Action::CloseWindow => {
                if let Some(mapped) = self.awm.layout.focus() {
                    mapped.toplevel().send_close();
                }
            }
            Action::CloseWindowById(id) => {
                let window = self.awm.layout.windows().find(|(_, m)| m.id().get() == id);
                if let Some((_, mapped)) = window {
                    mapped.toplevel().send_close();
                }
            }
            Action::FullscreenWindow => {
                let focus = self.awm.layout.focus().map(|m| m.window.clone());
                if let Some(window) = focus {
                    self.awm.layout.toggle_fullscreen(&window);
                    // FIXME: granular
                    self.awm.queue_redraw_all();
                }
            }
            ...
            Action::ToggleWindowedFullscreen => {
                let focus = self.awm.layout.focus().map(|m| m.window.clone());
                if let Some(window) = focus {
                    self.awm.layout.toggle_windowed_fullscreen(&window);
                    // FIXME: granular
                    self.awm.queue_redraw_all();
                }
            }
            ...
            Action::FocusWindowInColumn(index) => {
                self.awm.layout.focus_window_in_column(index);
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::FocusWindowPrevious => {
                let current = self.awm.layout.focus().map(|win| win.id());
                    ...
                    .map(|win| win.window.clone())
                {
                    ...
                    self.awm.mru_apply_keyboard_commit();

                    self.focus_window(&window);
                }
            }
            Action::SwitchLayout(action) => {
                let keyboard = &self.awm.seat.get_keyboard().unwrap();
            ...
            Action::MoveColumnLeft => {
                if self.awm.screenshot_ui.is_open() {
                    self.awm.screenshot_ui.move_left();
                } else {
                    self.awm.layout.move_left();
                    self.maybe_warp_cursor_to_focus();
                }
            ...
            Action::MoveColumnRight => {
                if self.awm.screenshot_ui.is_open() {
                    self.awm.screenshot_ui.move_right();
                } else {
                    self.awm.layout.move_right();
                    self.maybe_warp_cursor_to_focus();
                }
            ...
            Action::MoveColumnToFirst => {
                self.awm.layout.move_column_to_first();
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveColumnToLast => {
                self.awm.layout.move_column_to_last();
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveColumnLeftOrToMonitorLeft => {
                if self.awm.screenshot_ui.is_open() {
                    self.awm.screenshot_ui.move_left();
                } else if let Some(output) = self.awm.output_left() {
                    if self.awm.layout.move_column_left_or_to_output(&output)
            ...
            Action::MoveColumnRightOrToMonitorRight => {
                if self.awm.screenshot_ui.is_open() {
                    self.awm.screenshot_ui.move_right();
                } else if let Some(output) = self.awm.output_right() {
                    if self.awm.layout.move_column_right_or_to_output(&output)
                        && !self.maybe_warp_cursor_to_focus_centered()
                    {
                        self.move_cursor_to_output(&output);
                    } else {
            ...
            Action::MoveWindowDown => {
                if self.awm.screenshot_ui.is_open() {
                    self.awm.screenshot_ui.move_down();
                } else {
                    self.awm.layout.move_down();
                    self.maybe_warp_cursor_to_focus();
                }
            ...
            Action::MoveWindowUp => {
                if self.awm.screenshot_ui.is_open() {
                    self.awm.screenshot_ui.move_up();
                } else {
                    self.awm.layout.move_up();
                    self.maybe_warp_cursor_to_focus();
                }
            ...
            Action::MoveWindowDownOrToWorkspaceDown => {
                if self.awm.screenshot_ui.is_open() {
                    ...
                    self.awm.layout.move_down_or_to_workspace_down();
                    self.maybe_warp_cursor_to_focus();
                }
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::MoveWindowUpOrToWorkspaceUp => {
                if self.awm.screenshot_ui.is_open() {
                    ...
                    self.awm.layout.move_up_or_to_workspace_up();
                    self.maybe_warp_cursor_to_focus();
                }
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::ConsumeOrExpelWindowLeft => {
                self.awm.layout.consume_or_expel_window_left(None);
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::ConsumeOrExpelWindowLeftById(id) => {
                ...
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.consume_or_expel_window_left(Some(&window));
                    self.maybe_warp_cursor_to_focus();
                    // FIXME: granular
                    self.awm.queue_redraw_all();
                }
            ...
            Action::ConsumeOrExpelWindowRight => {
                self.awm.layout.consume_or_expel_window_right(None);
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            ...
            Action::FocusColumnLeftUnderMouse => {
                if let Some((output, ws)) = self.awm.workspace_under_cursor(true) {
                    let ws_id = ws.id();
                    let ws = {
                        let mut workspaces = self.awm.layout.workspaces_mut();
                        workspaces.find(|ws| ws.id() == ws_id).unwrap()
                    };
                    ws.focus_left();
                    self.maybe_warp_cursor_to_focus();
                    self.awm.layer_shell_on_demand_focus = None;
                    self.awm.queue_redraw(&output);
                }
            }
            Action::FocusColumnRight => {
                self.awm.layout.focus_right();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusColumnFirst => {
                self.awm.layout.focus_column_first();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusColumnLast => {
                self.awm.layout.focus_column_last();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusColumnRightOrFirst => {
                self.awm.layout.focus_column_right_or_first();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusColumnLeftOrLast => {
                self.awm.layout.focus_column_left_or_last();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusColumn(index) => {
                self.awm.layout.focus_column(index);
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowOrMonitorUp => {
                if let Some(output) = self.awm.output_up() {
                    if self.awm.layout.focus_window_up_or_output(&output)
                        && !self.maybe_warp_cursor_to_focus_centered()
                    {
                        self.move_cursor_to_output(&output);
                    } else {
            ...
            Action::FocusWindowOrMonitorDown => {
                if let Some(output) = self.awm.output_down() {
                    if self.awm.layout.focus_window_down_or_output(&output)
                        && !self.maybe_warp_cursor_to_focus_centered()
                    {
                        self.move_cursor_to_output(&output);
                    } else {
            ...
            Action::FocusColumnOrMonitorLeft => {
                if let Some(output) = self.awm.output_left() {
                    if self.awm.layout.focus_column_left_or_output(&output)
                        && !self.maybe_warp_cursor_to_focus_centered()
                    {
                        self.move_cursor_to_output(&output);
                    } else {
            ...
            Action::FocusColumnOrMonitorRight => {
                if let Some(output) = self.awm.output_right() {
                    if self.awm.layout.focus_column_right_or_output(&output)
                        && !self.maybe_warp_cursor_to_focus_centered()
                    {
                        self.move_cursor_to_output(&output);
                    } else {
            ...
            Action::FocusWindowDownOrColumnLeft => {
                self.awm.layout.focus_down_or_left();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowDownOrColumnRight => {
                self.awm.layout.focus_down_or_right();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowUpOrColumnLeft => {
                self.awm.layout.focus_up_or_left();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowUpOrColumnRight => {
                self.awm.layout.focus_up_or_right();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowOrWorkspaceDown => {
                self.awm.layout.focus_window_or_workspace_down();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowOrWorkspaceUp => {
                self.awm.layout.focus_window_or_workspace_up();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowTop => {
                self.awm.layout.focus_window_top();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowBottom => {
                self.awm.layout.focus_window_bottom();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowDownOrTop => {
                self.awm.layout.focus_window_down_or_top();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWindowUpOrBottom => {
                self.awm.layout.focus_window_up_or_bottom();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::MoveWindowToWorkspaceDown(focus) => {
                self.awm.layout.move_to_workspace_down(focus);
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveWindowToWorkspaceUp(focus) => {
                self.awm.layout.move_to_workspace_up(focus);
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveWindowToWorkspace(reference, focus) => {
                if let Some((mut output, index)) =
                    self.awm.find_output_and_workspace_index(reference)
                {
                    ...
                    if let Some(active) = self.awm.layout.active_output() {
                    ...
                    if let Some(output) = output {
                        ...
                        self.awm.layout.move_to_workspace(None, index, activate);
                        self.maybe_warp_cursor_to_focus();
                    }
            ...
            Action::MoveWindowToWorkspaceById {
                ...
                if let Some(window) = window {
                        ...
                        self.awm.find_output_and_workspace_index(reference)
                    {
                        ...
                        if let Some(output) = output {
                            self.awm.layout.move_to_output(
                                Some(&window),
                                &output,
                                Some(index),
                                activate,
                            ...
                            if !target_was_active
                                && self.awm.layout.active_output() == Some(&output)
                            {
                                if !self.maybe_warp_cursor_to_focus_centered() {
                                    self.move_cursor_to_output(&output);
                                }
                            ...
                            let new_focus = self.awm.layout.focus();
            ...
            Action::MoveColumnToWorkspaceDown(focus) => {
                self.awm.layout.move_column_to_workspace_down(focus);
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveColumnToWorkspaceUp(focus) => {
                self.awm.layout.move_column_to_workspace_up(focus);
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveColumnToWorkspace(reference, focus) => {
                    ...
                    self.awm.find_output_and_workspace_index(reference)
                {
                    ...
                    if let Some(output) = output {
                        ...
                        self.awm.layout.move_column_to_workspace(index, focus);
                        if focus {
                            self.maybe_warp_cursor_to_focus();
                        }
                    }
            ...
            Action::MoveColumnToIndex(idx) => {
                self.awm.layout.move_column_to_index(idx);
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::FocusWorkspaceDown => {
                self.awm.layout.switch_workspace_down();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWorkspaceDownUnderMouse => {
                if let Some(output) = self.awm.output_under_cursor() {
                    if let Some(mon) = self.awm.layout.monitor_for_output_mut(&output) {
            ...
            Action::FocusWorkspaceUp => {
                self.awm.layout.switch_workspace_up();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::FocusWorkspace(reference) => {
                    ...
                    self.awm.find_output_and_workspace_index(reference)
                {
                    ...
                    if let Some(output) = output {
                        self.awm.layout.focus_output(&output);
                        self.awm.layout.switch_workspace(index);
                        if !self.maybe_warp_cursor_to_focus_centered() {
                            self.move_cursor_to_output(&output);
                        }
                    } else {
                        let config = &self.awm.config;
                        if config.borrow().input.workspace_auto_back_and_forth {
                            self.awm.layout.switch_workspace_auto_back_and_forth(index);
                        } else {
                            self.awm.layout.switch_workspace(index);
                        }
                        self.maybe_warp_cursor_to_focus();
            ...
            Action::FocusWorkspacePrevious => {
                self.awm.layout.switch_workspace_previous();
                self.maybe_warp_cursor_to_focus();
                self.awm.layer_shell_on_demand_focus = None;
                // FIXME: granular
                self.awm.queue_redraw_all();
            ...
            Action::MoveWorkspaceDown => {
                self.awm.layout.move_workspace_down();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveWorkspaceUp => {
                self.awm.layout.move_workspace_up();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveWorkspaceToIndex(new_idx) => {
                let new_idx = new_idx.saturating_sub(1);
                self.awm.layout.move_workspace_to_idx(None, new_idx);
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveWorkspaceToIndexByRef { new_idx, reference } => {
                if let Some(res) = self.awm.find_output_and_workspace_index(reference) {
                    let new_idx = new_idx.saturating_sub(1);
                    self.awm.layout.move_workspace_to_idx(Some(res), new_idx);
                    // FIXME: granular
                    self.awm.queue_redraw_all();
                }
            }
            Action::SetWorkspaceName(name) => {
                self.awm.layout.set_workspace_name(name, None);
            }
            Action::SetWorkspaceNameByRef { name, reference } => {
                self.awm.layout.set_workspace_name(name, Some(reference));
            }
            Action::UnsetWorkspaceName => {
                self.awm.layout.unset_workspace_name(None);
            }
            Action::UnsetWorkSpaceNameByRef(reference) => {
                self.awm.layout.unset_workspace_name(Some(reference));
            }
            Action::ConsumeWindowIntoColumn => {
                self.awm.layout.consume_into_column();
            ...
            Action::ExpelWindowFromColumn => {
                self.awm.layout.expel_from_column();
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            ...
            Action::ToggleColumnTabbedDisplay => {
                self.awm.layout.toggle_column_tabbed_display();
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::SetColumnDisplay(display) => {
                self.awm.layout.set_column_display(display);
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::SwitchPresetColumnWidth => {
                self.awm.layout.toggle_width(true);
            }
            Action::SwitchPresetColumnWidthBack => {
                self.awm.layout.toggle_width(false);
            }
            Action::SwitchPresetWindowWidth => {
                self.awm.layout.toggle_window_width(None, true);
            }
            Action::SwitchPresetWindowWidthBack => {
                self.awm.layout.toggle_window_width(None, false);
            }
            Action::SwitchPresetWindowWidthById(id) => {
                let window = self.awm.layout.windows().find(|(_, m)| m.id().get() == id);
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.toggle_window_width(Some(&window), true);
                }
            }
            Action::SwitchPresetWindowWidthBackById(id) => {
                ...
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.toggle_window_width(Some(&window), false);
                }
            }
            Action::SwitchPresetWindowHeight => {
                self.awm.layout.toggle_window_height(None, true);
            }
            Action::SwitchPresetWindowHeightBack => {
                self.awm.layout.toggle_window_height(None, false);
            }
            Action::SwitchPresetWindowHeightById(id) => {
                let window = self.awm.layout.windows().find(|(_, m)| m.id().get() == id);
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.toggle_window_height(Some(&window), true);
                }
            }
            Action::SwitchPresetWindowHeightBackById(id) => {
                ...
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.toggle_window_height(Some(&window), false);
                }
            }
            Action::CenterColumn => {
                self.awm.layout.center_column();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::CenterWindow => {
                self.awm.layout.center_window(None);
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::CenterWindowById(id) => {
                ...
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.center_window(Some(&window));
                    // FIXME: granular
                    self.awm.queue_redraw_all();
                }
            }
            Action::CenterVisibleColumns => {
                self.awm.layout.center_visible_columns();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MaximizeColumn => {
                self.awm.layout.toggle_full_width();
            }
            Action::MaximizeWindowToEdges => {
                let focus = self.awm.layout.focus().map(|m| m.window.clone());
                if let Some(window) = focus {
                    self.awm.layout.toggle_maximized(&window);
                    // FIXME: granular
                    self.awm.queue_redraw_all();
                }
            }
            ...
            Action::FocusMonitorPrevious => {
                if let Some(output) = self.awm.output_previous() {
                    self.awm.layout.focus_output(&output);
                    if !self.maybe_warp_cursor_to_focus_centered() {
                        self.move_cursor_to_output(&output);
                    }
            ...
            Action::FocusMonitorNext => {
                if let Some(output) = self.awm.output_next() {
                    self.awm.layout.focus_output(&output);
                    if !self.maybe_warp_cursor_to_focus_centered() {
                        self.move_cursor_to_output(&output);
                    }
            ...
            Action::FocusMonitor(output) => {
                if let Some(output) = self.awm.output_by_name_match(&output).cloned() {
                    self.awm.layout.focus_output(&output);
                    if !self.maybe_warp_cursor_to_focus_centered() {
                        self.move_cursor_to_output(&output);
                    }
            ...
            Action::MoveWindowToMonitorLeft => {
                if let Some(current_output) = self.awm.screenshot_ui.selection_output() {
                    if let Some(target_output) = self.awm.output_left_of(current_output) {
                        self.move_cursor_to_output(&target_output);
                        self.awm.screenshot_ui.move_to_output(target_output);
                    }
                } else if let Some(output) = self.awm.output_left() {
                    self.awm
                        .layout
            ...
            Action::MoveWindowToMonitorRight => {
                if let Some(current_output) = self.awm.screenshot_ui.selection_output() {
                    if let Some(target_output) = self.awm.output_right_of(current_output) {
                        self.move_cursor_to_output(&target_output);
                        self.awm.screenshot_ui.move_to_output(target_output);
                    }
                } else if let Some(output) = self.awm.output_right() {
            ...
            Action::MoveWindowToMonitorDown => {
                if let Some(current_output) = self.awm.screenshot_ui.selection_output() {
                    if let Some(target_output) = self.awm.output_down_of(current_output) {
                        self.move_cursor_to_output(&target_output);
                        self.awm.screenshot_ui.move_to_output(target_output);
                    }
                } else if let Some(output) = self.awm.output_down() {
                    self.awm
                        .layout
                        .move_to_output(None, &output, None, ActivateWindow::Smart);
                    self.awm.layout.focus_output(&output);
            ...
            Action::MoveWindowToMonitorUp => {
                if let Some(current_output) = self.awm.screenshot_ui.selection_output() {
                    if let Some(target_output) = self.awm.output_up_of(current_output) {
                        self.move_cursor_to_output(&target_output);
                        self.awm.screenshot_ui.move_to_output(target_output);
                    }
                } else if let Some(output) = self.awm.output_up() {
                    self.awm
                        .layout
                        .move_to_output(None, &output, None, ActivateWindow::Smart);
                    self.awm.layout.focus_output(&output);
            ...
            Action::MoveWindowToMonitorPrevious => {
                if let Some(current_output) = self.awm.screenshot_ui.selection_output() {
                    if let Some(target_output) = self.awm.output_previous_of(current_output) {
                        self.move_cursor_to_output(&target_output);
                        self.awm.screenshot_ui.move_to_output(target_output);
                    }
                } else if let Some(output) = self.awm.output_previous() {
                    self.awm
                        .layout
                        .move_to_output(None, &output, None, ActivateWindow::Smart);
                    self.awm.layout.focus_output(&output);
            ...
            Action::MoveWindowToMonitorNext => {
                if let Some(current_output) = self.awm.screenshot_ui.selection_output() {
                    if let Some(target_output) = self.awm.output_next_of(current_output) {
                        self.move_cursor_to_output(&target_output);
                        self.awm.screenshot_ui.move_to_output(target_output);
                    }
                } else if let Some(output) = self.awm.output_next() {
                    self.awm
                        .layout
                        .move_to_output(None, &output, None, ActivateWindow::Smart);
                    self.awm.layout.focus_output(&output);
            ...
            Action::MoveWindowToMonitor(output) => {
                if let Some(output) = self.awm.output_by_name_match(&output).cloned() {
                    if self.awm.screenshot_ui.is_open() {
                        self.move_cursor_to_output(&output);
                        self.awm.screenshot_ui.move_to_output(output);
                    } else {
                        self.awm
                            .layout
                            .move_to_output(None, &output, None, ActivateWindow::Smart);
            ...
            Action::MoveWindowToMonitorById { id, output } => {
                if let Some(output) = self.awm.output_by_name_match(&output).cloned() {
                    ...
                    if let Some(window) = window {
                        ...
                        // If the active output changed (window was moved and focused).
                        #[allow(clippy::collapsible_if)]
                        if !target_was_active && self.awm.layout.active_output() == Some(&output) {
                            if !self.maybe_warp_cursor_to_focus_centered() {
                                self.move_cursor_to_output(&output);
                            }
                        }
            ...
            Action::MoveColumnToMonitorLeft => {
                if let Some(current_output) = self.awm.screenshot_ui.selection_output() {
                    ...
                    self.awm.layout.move_column_to_output(&output, None, true);
                    self.awm.layout.focus_output(&output);
                    if !self.maybe_warp_cursor_to_focus_centered() {
                        self.move_cursor_to_output(&output);
                    }
            ...
            Action::SetColumnWidth(change) => {
                if self.awm.screenshot_ui.is_open() {
                    self.awm.screenshot_ui.set_width(change);

                    // FIXME: granular
                    self.awm.queue_redraw_all();
                } else {
                    self.awm.layout.set_column_width(change);
                }
            }
            Action::SetWindowWidth(change) => {
                if self.awm.screenshot_ui.is_open() {
                    ...
                    self.awm.layout.set_window_width(None, change);
                }
            }
            Action::SetWindowWidthById { id, change } => {
                ...
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.set_window_width(Some(&window), change);
                }
            }
            Action::SetWindowHeight(change) => {
                if self.awm.screenshot_ui.is_open() {
                    self.awm.screenshot_ui.set_height(change);

                    // FIXME: granular
                    self.awm.queue_redraw_all();
                } else {
                    self.awm.layout.set_window_height(None, change);
                }
            }
            Action::SetWindowHeightById { id, change } => {
                ...
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.set_window_height(Some(&window), change);
                }
            }
            Action::ResetWindowHeight => {
                self.awm.layout.reset_window_height(None);
            }
            Action::ResetWindowHeightById(id) => {
                let window = self.awm.layout.windows().find(|(_, m)| m.id().get() == id);
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.reset_window_height(Some(&window));
                }
            }
            Action::ExpandColumnToAvailableWidth => {
                self.awm.layout.expand_column_to_available_width();
            }
            Action::ShowHotkeyOverlay => {
                if self.awm.hotkey_overlay.show() {
                    self.awm.queue_redraw_all();

                    #[cfg(feature = "dbus")]
                    self.awm.a11y_announce_hotkey_overlay();
                }
            }
            Action::MoveWorkspaceToMonitorLeft => {
                if let Some(output) = self.awm.output_left() {
                    self.awm.layout.move_workspace_to_output(&output);
                    if !self.maybe_warp_cursor_to_focus_centered() {
                        self.move_cursor_to_output(&output);
                    }
                }
            ...
            Action::MoveWorkspaceToMonitor(new_output) => {
                if let Some(new_output) = self.awm.output_by_name_match(&new_output).cloned() {
                    if self.awm.layout.move_workspace_to_output(&new_output)
                        && !self.maybe_warp_cursor_to_focus_centered()
                    {
            ...
            Action::MoveWorkspaceToMonitorByRef {
                ...
                if let Some((output, old_idx)) =
                    self.awm.find_output_and_workspace_index(reference)
                {
                    if let Some(new_output) = self.awm.output_by_name_match(&output_name).cloned()
                    {
                        if self.awm.layout.move_workspace_to_output_by_id(
                            old_idx,
                            output,
                            &new_output,
                        ) {
            ...
            Action::ToggleWindowFloating => {
                self.awm.layout.toggle_window_floating(None);
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::ToggleWindowFloatingById(id) => {
                ...
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.toggle_window_floating(Some(&window));
                    // FIXME: granular
                    self.awm.queue_redraw_all();
                }
            }
            Action::MoveWindowToFloating => {
                self.awm.layout.set_window_floating(None, true);
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveWindowToFloatingById(id) => {
                ...
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.set_window_floating(Some(&window), true);
                    // FIXME: granular
                    self.awm.queue_redraw_all();
                }
            }
            Action::MoveWindowToTiling => {
                self.awm.layout.set_window_floating(None, false);
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::MoveWindowToTilingById(id) => {
                ...
                let window = window.map(|(_, m)| m.window.clone());
                if let Some(window) = window {
                    self.awm.layout.set_window_floating(Some(&window), false);
                    // FIXME: granular
                    self.awm.queue_redraw_all();
                }
            }
            Action::FocusFloating => {
                self.awm.layout.focus_floating();
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::FocusTiling => {
                self.awm.layout.focus_tiling();
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            Action::SwitchFocusBetweenFloatingAndTiling => {
                self.awm.layout.switch_focus_floating_tiling();
                self.maybe_warp_cursor_to_focus();
                // FIXME: granular
                self.awm.queue_redraw_all();
            }
            ...
            Action::SetDynamicCastMonitor(output) => {
                let output = match output {
                    None => self.awm.layout.active_output(),
                    Some(name) => self.awm.output_by_name_match(&name),
                };
                if let Some(output) = output {
            ...
            Action::StopCast(session_id) => {
                self.awm.stop_cast(CastSessionId::from(session_id));
            }
            Action::ToggleOverview => {
                self.awm.layout.toggle_overview();
                self.awm.queue_redraw_all();
            }
            Action::OpenOverview => {
                if self.awm.layout.open_overview() {
                    self.awm.queue_redraw_all();
                }
            }
            Action::CloseOverview => {
                if self.awm.layout.close_overview() {
                    self.awm.queue_redraw_all();
                }
            }
            ...
            Action::LoadConfigFile(path) => {
                if let Some(watcher) = &self.awm.config_file_watcher {
            ...
            Action::MruAdvance {
                ...
                if self.awm.window_mru_ui.is_open() {
                    self.awm.window_mru_ui.advance(direction, filter);
                    self.awm.queue_redraw_mru_output();
                } else if self.awm.config.borrow().recent_windows.on {
                    self.awm.mru_apply_keyboard_commit();

                    let config = self.awm.config.borrow();
                    let scope = scope.unwrap_or(self.awm.window_mru_ui.scope());

                    let mut wmru = WindowMru::new(&self.awm);
                    if !wmru.is_empty() {
                        ...
                        if let Some(output) = self.awm.layout.active_output() {
                            self.awm.window_mru_ui.open(
                                self.awm.clock.clone(),
                                wmru,
                                output.clone(),
                            );
                            ...
                            let keep_first = direction == MruDirection::Forward
                                && self.awm.layout.focus().is_none();
                            if !keep_first {
                                self.awm.window_mru_ui.advance(direction, None);
                            }
            ...
            Action::MruCloseCurrentWindow => {
                if self.awm.window_mru_ui.is_open() {
                    if let Some(id) = self.awm.window_mru_ui.current_window_id() {
                        if let Some(w) = self.awm.find_window_by_id(id) {
            ...
            Action::MruFirst => {
                if self.awm.window_mru_ui.is_open() {
                    self.awm.window_mru_ui.first();
                    self.awm.queue_redraw_mru_output();
                }
            }
            Action::MruLast => {
                if self.awm.window_mru_ui.is_open() {
                    self.awm.window_mru_ui.last();
                    self.awm.queue_redraw_mru_output();
                }
            }
            Action::MruSetScope(scope) => {
                if self.awm.window_mru_ui.is_open() {
                    self.awm.window_mru_ui.set_scope(scope);
                    self.awm.queue_redraw_mru_output();
                }
            }
            Action::MruCycleScope => {
                if self.awm.window_mru_ui.is_open() {
                    self.awm.window_mru_ui.cycle_scope();
                    self.awm.queue_redraw_mru_output();
                }
            }
        }
    ...
    fn on_pointer_motion<I: InputBackend>(&mut self, event: I::PointerMotionEvent) {
        let was_inside_hot_corner = self.awm.pointer_inside_hot_corner;
        // Any of the early returns here mean that the pointer is not inside the hot corner.
        self.awm.pointer_inside_hot_corner = false;

        // We need an output to be able to move the pointer.
        if self.awm.global_space.outputs().next().is_none() {
            return;
        }

        let serial = SERIAL_COUNTER.next_serial();

        let pointer = self.awm.seat.get_pointer().unwrap();
        ...
        self.awm.pointer_visibility = PointerVisibility::Visible;
        self.awm.tablet_cursor_location = None;
        ...
        if let Some(under) = &self.awm.pointer_contents.surface {
        ...
        if let Some((output, horizontal)) = spatial_grab.flatten() {
            if let Some(geo) = self.awm.global_space.output_geometry(&output) {
                let geo = geo.to_f64();
                if horizontal {
            ...
            .is_none()
        {
            ...
            if let Some(output) = self.awm.global_space.output_under(pos).next() {
                // The pointer was previously on some output. Clip the movement against its
                // boundaries.
                let geom = self.awm.global_space.output_geometry(output).unwrap();
                new_pos.x = new_pos
                    .x
        ...
        if let Some(output) = self.awm.screenshot_ui.selection_output() {
            let geom = self.awm.global_space.output_geometry(output).unwrap();
            let point = (new_pos - geom.loc.to_f64())
                .to_physical(output.current_scale().fractional_scale())
                .to_i32_round::<i32>();

            self.awm.screenshot_ui.pointer_motion(point, None);
        }

        if let Some(mru_output) = self.awm.window_mru_ui.output() {
            if let Some((output, pos_within_output)) = self.awm.output_under(new_pos) {
                if mru_output == output {
                    self.awm.window_mru_ui.pointer_motion(pos_within_output);
                }
            }
        }

        let under = self.awm.contents_under(new_pos);
        ...
        self.awm.handle_focus_follows_mouse(&under);

        self.awm.pointer_contents.clone_from(&under);
        ...
        if under.hot_corner && pointer.current_focus().is_none() {
            ...
            self.awm.pointer_inside_hot_corner = true;
        }

        // Activate a new confinement if necessary.
        self.awm.maybe_activate_pointer_constraint();
        ...
        if is_dnd_grab {
            if let Some((output, pos_within_output)) = self.awm.output_under(new_pos) {
                ...
                self.awm.layout.dnd_update(output, pos_within_output);
            }
        }
    ...
    fn on_pointer_motion_absolute<I: InputBackend>(
        ...
        if let Some(mru_output) = self.awm.window_mru_ui.output() {
            if let Some((output, pos_within_output)) = self.awm.output_under(pos) {
                if mru_output == output {
                    self.awm.window_mru_ui.pointer_motion(pos_within_output);
                }
            }
        ...
        let under = self.awm.contents_under(pos);
    ...
    fn on_pointer_button<I: InputBackend>(&mut self, event: I::PointerButtonEvent) {
        ...
        if self.awm.suppressed_buttons.remove(&button_code) {
            return;
        }

        let mods = self.awm.seat.get_keyboard().unwrap().modifier_state();
        let modifiers = modifiers_from_state(mods);
        ...
        if ButtonState::Pressed == button_state {
            ...
            if let Some(mru_output) = self.awm.window_mru_ui.output() {
                ...
                if let Some(MouseButton::Left) = button {
                    ...
                    let (output, pos_within_output) = self.awm.output_under(location).unwrap();
                    if mru_output == output {
                        let id = self.awm.window_mru_ui.pointer_motion(pos_within_output);
                        if id.is_some() {
                            self.confirm_mru();
                        } else {
                            self.awm.cancel_mru();
                    ...
                    self.awm.suppressed_buttons.insert(button_code);
                    return;
                }
            }

            if is_mru_open || self.awm.mods_with_mouse_binds.contains(&modifiers) {
                ...
                .and_then(|trigger| {
                    let config = self.awm.config.borrow();
                    let bindings =
                        make_binds_iter(&config, &mut self.awm.window_mru_ui, modifiers);
                    find_configured_bind(bindings, mod_key, trigger, mods)
                })
                .filter(|bind| {
                    !self.awm.screenshot_ui.is_open() || allowed_during_screenshot(&bind.action)
                }) {
                    self.awm.suppressed_buttons.insert(button_code);
                    self.handle_bind(bind.clone());
                    return;
            ...
            let is_overview_open = self.awm.layout.is_overview_open();

            if is_overview_open && !pointer.is_grabbed() && button == Some(MouseButton::Right) {
                if let Some((output, ws)) = self.awm.workspace_under_cursor(true) {
                    let ws_id = ws.id();
                    let ws_idx = self.awm.layout.find_workspace_by_id(ws_id).unwrap().0;
            ...
            if button == Some(MouseButton::Middle) && !pointer.is_grabbed() && mod_down {
                let output_ws = if is_overview_open {
                    self.awm.workspace_under_cursor(true)
                } else {
                    // We don't want to accidentally "catch" the wrong workspace during
                    // animations.
                    self.awm.output_under_cursor().and_then(|output| {
                        let mon = self.awm.layout.monitor_for_output(&output)?;
            ...
            if let Some(mapped) = self.awm.window_under_cursor() {
                ...
                if button == Some(MouseButton::Left) && !pointer.is_grabbed() {
                    if is_overview_open || mod_down {
                        ...
                        if !is_overview_open {
                            self.awm.layout.activate_window(&window);
                        }
                ...
                .then(|| self.awm.workspace_under_cursor(false))
                .flatten()
            {
                let ws_idx = self.awm.layout.find_workspace_by_id(ws.id()).unwrap().0;

                self.awm.layout.focus_output(&output);
                self.awm.layout.toggle_overview_to_workspace(ws_idx);

                // FIXME: granular.
                self.awm.queue_redraw_all();
            } else if let Some(output) = self.awm.output_under_cursor() {
                self.awm.layout.focus_output(&output);
        ...
        if ButtonState::Pressed == button_state {
            let layer_under = self.awm.pointer_contents.layer.clone();
            self.awm.focus_layer_surface_if_on_demand(layer_under);
        }

        if button == Some(MouseButton::Left) && self.awm.screenshot_ui.is_open() {
            if button_state == ButtonState::Pressed {
                let pos = pointer.current_location();

                // If we'll be moving the existing selection, use the selection output.
                let output = if mod_down {
                    self.awm.screenshot_ui.selection_output()
                } else {
                    self.awm.output_under(pos).map(|(out, _)| out)
                };
            ...
            } else if let Some(capture) = self.awm.screenshot_ui.pointer_up(None) {
                if capture {
    ...
    fn on_pointer_axis<I: InputBackend>(&mut self, event: I::PointerAxisEvent) {
        ...
        let should_handle_in_overview = if is_overview_open {
            ...
            pointer
                .current_focus()
                .map(|surface| self.awm.find_root_shell_surface(&surface))
                .is_none_or(|root| {
                    !self
                        .awm
        ...
        let is_mru_open = self.awm.window_mru_ui.is_open();
        ...
        if source == AxisSource::Wheel {
            ...
            let should_handle = should_handle_in_overview
                || is_mru_open
                || self.awm.mods_with_wheel_binds.contains(&modifiers);
            if should_handle {
                let horizontal = horizontal_amount_v120.unwrap_or(0.);
                let ticks = self.awm.horizontal_wheel_tracker.accumulate(horizontal);
                if ticks != 0 {
                    let (bind_left, bind_right) =
                        if should_handle_in_overview && modifiers.is_empty() {
                            ...
                            .filter(|bind| {
                                !self.awm.screenshot_ui.is_open()
                ...
                let ticks = self.awm.vertical_wheel_tracker.accumulate(vertical);
                if ticks != 0 {
                ...
                self.awm.horizontal_wheel_tracker.reset();
                self.awm.vertical_wheel_tracker.reset();
            }
        }
        ...
        if source == AxisSource::Finger || source == AxisSource::Continuous {
            ...
            if should_handle_in_overview && modifiers.is_empty() {
                ...
                let is_vertical = self.awm.overview_scroll_swipe_gesture.is_vertical();
                    ...
                    if is_vertical {
                        ...
                        if action.begin() {
                            if let Some((output, ws)) = self.awm.workspace_under_cursor(true) {
                                let ws_id = ws.id();
                                let ws_idx =
                                    self.awm.layout.find_workspace_by_id(ws_id).unwrap().0;

                                self.awm.layout.view_offset_gesture_begin(
                                    &output,
                                    Some(ws_idx),
                                    true,
                                );
                ...
                let mut redraw = false;
                if self.awm.overview_scroll_swipe_gesture.reset() {
                    if self.awm.overview_scroll_swipe_gesture.is_vertical() {
                        redraw |= self
                            .awm
                            .layout
                            .workspace_switch_gesture_end(Some(true))
            ...
            if is_mru_open || self.awm.mods_with_finger_scroll_binds.contains(&modifiers) {
                let ticks = self
                    .awm
                ...
                self.awm.horizontal_finger_scroll_tracker.reset();
                self.awm.vertical_finger_scroll_tracker.reset();
            }
        }
        ...
        let window_scroll_factor = pointer
            .current_focus()
            .map(|focused| self.awm.find_root_shell_surface(&focused))
            .and_then(|root| self.awm.layout.find_window_and_output(&root).unzip().0)
        ...
        I::Device: 'static, // Needed for downcasting.
    {
        ...
        if let Some(tool) = tool {
            ...
            self.awm.pointer_visibility = PointerVisibility::Visible;
            self.awm.tablet_cursor_location = Some(pos);
        }
    ...
    fn on_tablet_tool_tip<I: InputBackend>(&mut self, event: I::TabletToolTipEvent) {
        let tool = self.awm.seat.tablet_seat().get_tool(&event.tool());
        ...
        match tip_state {
            TabletToolTipState::Down => {
                if let Some(pos) = self.awm.tablet_cursor_location {
                    let under = self.awm.contents_under(pos);
                    ...
                    if self.awm.screenshot_ui.is_open() {
                    ...
                    } else if let Some(mru_output) = self.awm.window_mru_ui.output() {
                        if let Some((output, pos_within_output)) = self.awm.output_under(pos) {
                            if mru_output == output {
                                let id = self.awm.window_mru_ui.pointer_motion(pos_within_output);
                                if id.is_some() {
                        ...
                        if self.awm.layout.is_overview_open()
                            && !mod_down
                            && under.layer.is_none()
                            && under.output.is_some()
                        {
                            let (output, pos_within_output) = self.awm.output_under(pos).unwrap();
                            let output = output.clone();

                            let mut matched_narrow = true;
                            let mut ws = self.awm.workspace_under(false, pos);
                            if ws.is_none() {
                                matched_narrow = false;
                                ws = self.awm.workspace_under(true, pos);
                            }
                            let ws_id = ws.map(|(_, ws)| ws.id());

                            let mapped = self.awm.window_under(pos);
                        ...
                        self.awm.focus_layer_surface_if_on_demand(under.layer);
                    }
                }
            ...
            TabletToolTipState::Up => {
                if let Some(capture) = self.awm.screenshot_ui.pointer_up(None) {
                    if capture {
                        self.confirm_screenshot(true);
                    } else {
                        self.awm.queue_redraw_all();
        ...
        I::Device: 'static, // Needed for downcasting.
    {
        ...
        let tablet_seat = self.awm.seat.tablet_seat();
        let display_handle = self.awm.display_handle.clone();
    ...
    fn on_tablet_tool_button<I: InputBackend>(&mut self, event: I::TabletToolButtonEvent) {
        ...
        if let Some(tool) = tool {
            ...
            if self.awm.suppressed_buttons.remove(&button) {
                return;
            }
            ...
            if let Some(trigger) = trigger {
                if event.button_state() == ButtonState::Pressed {
                    ...
                    if self.awm.mods_with_tablet_stylus_binds.contains(&modifiers) {
                        let bind = {
                            let config = self.awm.config.borrow();
                        ...
                        if let Some(bind) = bind {
                            self.awm.suppressed_buttons.insert(button);
                            self.handle_bind(bind.clone());
                            return;
                        }
                    }
    ...
    fn on_gesture_swipe_begin<I: InputBackend>(&mut self, event: I::GestureSwipeBeginEvent) {
        ...
        if event.fingers() == 3 {
            self.awm.gesture_swipe_3f_cumulative = Some((0., 0.));

            // We handled this event.
            return;
        } else if event.fingers() == 4 {
            self.awm.layout.overview_gesture_begin();
            self.awm.queue_redraw_all();
        ...
        I::Device: 'static,
    {
        ...
        if let Some((cx, cy)) = &mut self.awm.gesture_swipe_3f_cumulative {
            *cx += delta_x;
            *cy += delta_y;
            ...
            if cx * cx + cy * cy >= 16. * 16. {
                self.awm.gesture_swipe_3f_cumulative = None;
    ...
    fn on_gesture_swipe_end<I: InputBackend>(&mut self, event: I::GestureSwipeEndEvent) {
        ...
        let mut handled = false;
        let res = self.awm.layout.workspace_switch_gesture_end(Some(true));
        if let Some(output) = res {
            self.awm.queue_redraw(&output);
            handled = true;
        }

        let res = self.awm.layout.view_offset_gesture_end(Some(true));
        if let Some(output) = res {
            self.awm.queue_redraw(&output);
            handled = true;
        }

        let res = self.awm.layout.overview_gesture_end();
        if res {
            self.awm.queue_redraw_all();
            handled = true;
        }
    ...
    fn compute_absolute_location<I: InputBackend>(
        ...
        let output = output.filter(|output| self.awm.output_exists(output));
        let output = output.as_ref().or(fallback_output)?;
        let output_geo = self.awm.global_space.output_geometry(output).unwrap();
    ...
    fn compute_touch_location<I: InputBackend>(
        ...
        self.compute_absolute_location(evt, self.awm.output_for_touch())
    }

    fn on_touch_down<I: InputBackend>(&mut self, evt: I::TouchDownEvent) {
        let Some(handle) = self.awm.seat.get_touch() else {
            return;
        };
        ...
        self.awm.pointer_visibility = PointerVisibility::Disabled;
    }
    fn on_touch_up<I: InputBackend>(&mut self, evt: I::TouchUpEvent) {
        ...
        if let Some(capture) = self.awm.screenshot_ui.pointer_up(Some(slot)) {
            if capture {
                self.confirm_screenshot(true);
            } else {
                self.awm.queue_redraw_all();
    ...
    fn on_touch_motion<I: InputBackend>(&mut self, evt: I::TouchMotionEvent) {
        ...
        if let Some(output) = self.awm.screenshot_ui.selection_output().cloned() {
            let geom = self.awm.global_space.output_geometry(&output).unwrap();
            let point = (pos - geom.loc.to_f64())
                .to_physical(output.current_scale().fractional_scale())
                .to_i32_round::<i32>();

            self.awm.screenshot_ui.pointer_motion(point, Some(slot));
            self.awm.queue_redraw(&output);
        }
    ...
    fn on_switch_toggle<I: InputBackend>(&mut self, evt: I::SwitchToggleEvent) {
        ...
        let action = {
            let bindings = &self.awm.config.borrow().switch_events;
...
pub fn apply_libinput_settings(config: &awm_config::Input, device: &mut input::Device) {
    ...
    if is_touchpad {
        ...
        if let Some(method) = c.scroll_method {
            ...
            if method == awm_config::ScrollMethod::OnButtonDown {
