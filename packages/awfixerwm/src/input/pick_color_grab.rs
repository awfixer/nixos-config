...
impl PickColorGrab {
    ...
    fn on_ungrab(&mut self, state: &mut State) {
        if let Some(tx) = state.awm.pick_color.take() {
    ...
    fn pick_color_at_point(location: Point<f64, Logical>, data: &mut State) -> Option<PickedColor> {
        let (output, pos_within_output) = data.awm.output_under(location)?;
        let output = output.clone();

        data.backend
            .with_primary_renderer(|renderer| {
                data.awm.update_render_elements(Some(&output));
                ...
                let elements = data.awm.render_to_vec(ctx, &output, false);
...
impl PointerGrab<State> for PickColorGrab {
    ...
    fn button(
        ...
        data.awm.suppressed_buttons.insert(event.button);

        if let Some(tx) = data.awm.pick_color.take() {
