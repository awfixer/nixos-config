...
use awm_config::debug::PreviewRender;
use awm_config::output::MaxBpc;
use awm_config::{
...
#[cfg(feature = "dbus")]
use crate::dbus::freedesktop_locale1::Locale1ToAWM;
#[cfg(feature = "dbus")]
use crate::dbus::freedesktop_login1::Login1ToAWM;
#[cfg(feature = "dbus")]
use crate::dbus::gnome_shell_introspect::{self, IntrospectToAWM, AWMToIntrospect};
#[cfg(feature = "dbus")]
use crate::dbus::gnome_shell_screenshot::{AWMToScreenshot, ScreenshotToAWM};
...
use crate::render_helpers::renderer::AWMRenderer;
...
pub struct AWM {
    ...
    pub config_file_output_config: awm_config::Outputs,
    ...
    /// Whether awm was run with `--session`
    pub is_session_instance: bool,

    /// Name of the Wayland socket.
    ///
    /// This is `None` when creating `AWM` without a Wayland socket.
    ...
    // unredirected.
    //
    // https://github.com/awfixerwm/awm/issues/619
    #[cfg(test)]
    ...
    pub pick_color: Option<async_channel::Sender<Option<awm_ipc::PickedColor>>>,
...
// The surfaces here are always toplevel surfaces focused as far as awm's logic is concerned, even
...
pub struct PointContents {
    ...
    //
    // Can be `None` even when `window` is set, for example when the pointer is over the awm
...
impl CastTarget {
    ...
    pub fn matches(&self, ipc: &awm_ipc::CastTarget) -> bool {
        use CastTarget::*;
        match (self, ipc) {
            (Nothing, awm_ipc::CastTarget::Nothing {}) => true,
            (Output { name, .. }, awm_ipc::CastTarget::Output { name: ipc_name }) => {
                name == ipc_name
            }
            (Window { id }, awm_ipc::CastTarget::Window { id: ipc_id }) => id == ipc_id,
            _ => false,
        }
    }

    pub fn make_ipc(&self) -> awm_ipc::CastTarget {
        use CastTarget::*;
        match self {
            Nothing => awm_ipc::CastTarget::Nothing {},
            Output { name, .. } => awm_ipc::CastTarget::Output { name: name.clone() },
            Window { id } => awm_ipc::CastTarget::Window { id: *id },
        }
    }
}
...
impl State {
    pub fn new(
        ...
        backend.init(&mut awm);

        let mut state = Self { backend, awm };
    ...
    pub fn refresh_and_flush_clients(&mut self) {
        ...
        self.awm.redraw_queued_outputs(&mut self.backend);

        {
            let _span = tracy_client::span!("flush_clients");
            self.awm.display_handle.flush_clients().unwrap();
        }

        #[cfg(feature = "dbus")]
        self.awm.update_locked_hint();

        // Clear the time so it's fetched afresh next iteration.
        self.awm.clock.clear();
        self.awm.pointer_inactivity_timer_got_reset = false;
        self.awm.notified_activity_this_iteration = false;
    }
    ...
    pub fn set_lid_closed(&mut self, is_closed: bool) {
        if self.awm.is_lid_closed == is_closed {
            return;
        }

        debug!("laptop lid {}", if is_closed { "closed" } else { "opened" });
        self.awm.is_lid_closed = is_closed;
        self.backend.on_output_config_changed(&mut self.awm);
    }
    ...
    fn refresh(&mut self) {
        ...
        self.awm.refresh_window_states();

        // Needs to be called after updating the keyboard focus.
        self.awm.refresh_layout();

        self.awm.cursor_manager.check_cursor_image_surface_alive();
        self.awm.refresh_pointer_outputs();
        self.awm.global_space.refresh();
        self.awm.refresh_idle_inhibit();
        self.refresh_pointer_contents();
        foreign_toplevel::refresh(self);
        ext_workspace::refresh(self);

        #[cfg(feature = "xdp-gnome-screencast")]
        self.awm.refresh_mapped_cast_outputs();
        // Should happen before refresh_window_rules(), but after anything that can start or stop
        // screencasts.
        #[cfg(feature = "xdp-gnome-screencast")]
        self.awm.refresh_mapped_cast_window_rules();
        self.ipc_refresh_casts();

        self.awm.refresh_window_rules();
        ...
        // Needs to be called after updating the keyboard focus.
        #[cfg(feature = "dbus")]
        self.awm.refresh_a11y();
    }

    fn notify_blocker_cleared(&mut self) {
        let dh = self.awm.display_handle.clone();
        while let Ok(client) = self.awm.blocker_cleared_rx.try_recv() {
    ...
    pub fn move_cursor(&mut self, location: Point<f64, Logical>) {
        let mut under = match self.awm.pointer_visibility {
            PointerVisibility::Disabled => PointContents::default(),
            _ => self.awm.contents_under(location),
        };

        // Disable the hidden pointer if the contents underneath have changed.
        if !self.awm.pointer_visibility.is_visible() && self.awm.pointer_contents != under {
    ...
    pub fn move_cursor_to_focused_tile(&mut self, mode: CenterCoords) -> bool {
        if !self.awm.keyboard_focus.is_layout() {
            return false;
        }
        ...
        let Some(output) = self.awm.layout.active_output() else {
            return false;
        };
        let monitor = self.awm.layout.monitor_for_output(output).unwrap();
    ...
    pub fn focus_default_monitor(&mut self) {
        ...
        let Some(mut target) = self.awm.sorted_outputs.first().cloned() else {
        ...
        for config in &config.outputs.0 {
            ...
            if let Some(output) = self.awm.output_by_name_match(&config.name) {
                target = output.clone();
                break;
            }
        }
        ...
        self.awm.layout.focus_output(&target);
    ...
    pub fn focus_window(&mut self, window: &Window) {
        let active_output = self.awm.layout.active_output().cloned();

        self.awm.layout.activate_window(window);

        let new_active = self.awm.layout.active_output().cloned();
    ...
    pub fn confirm_mru(&mut self) {
        if let Some(window) = self.awm.close_mru(MruCloseRequest::Confirm) {
    ...
    pub fn maybe_warp_cursor_to_focus(&mut self) -> bool {
        let focused = match self.awm.config.borrow().input.warp_mouse_to_focus {
            None => return false,
    ...
    pub fn refresh_pointer_contents(&mut self) {
        ...
        let _span = tracy_client::span!("AWM::refresh_pointer_contents");

        let pointer = &self.awm.seat.get_pointer().unwrap();
        let location = pointer.current_location();

        if !self.awm.exit_confirm_dialog.is_open()
            && !self.awm.is_locked()
            && !self.awm.screenshot_ui.is_open()
        {
            // Don't refresh cursor focus during transitions.
            if let Some((output, _)) = self.awm.output_under(location) {
                let monitor = self.awm.layout.monitor_for_output(output).unwrap();
    ...
    pub fn update_pointer_contents(&mut self) -> bool {
        let _span = tracy_client::span!("AWM::update_pointer_contents");
        ...
        if self.awm.pointer_contents == under {
            return false;
        }
    ...
    pub fn refresh_popup_grab(&mut self) {
        if let Some(grab) = &mut self.awm.popup_grab {
            if grab.grab.has_ended() {
                self.awm.popup_grab = None;
            }
        }
    }
    ...
    pub fn update_keyboard_focus(&mut self) {
        ...
        if let Some(surface) = &self.awm.layer_shell_on_demand_focus {
            // Still alive and has on-demand interactivity.
            let mut good = surface.alive()
                && surface.cached_state().keyboard_interactivity
                    == wlr_layer::KeyboardInteractivity::OnDemand;

            if let Some(mapped) = self.awm.mapped_layer_surfaces.get(surface) {
        ...
        let focus = if self.awm.exit_confirm_dialog.is_open() {
            KeyboardFocus::ExitConfirmDialog
        } else if self.awm.is_locked() {
            KeyboardFocus::LockScreen {
                surface: self.awm.lock_surface_focus(),
            }
        } else if self.awm.screenshot_ui.is_open() {
            KeyboardFocus::ScreenshotUi
        } else if self.awm.window_mru_ui.is_open() {
            KeyboardFocus::Mru
        } else if let Some(output) = self.awm.layout.active_output() {
            let mon = self.awm.layout.monitor_for_output(output).unwrap();
            ...
            let layer_grab = self.awm.popup_grab.as_ref().and_then(|g| {
                layers
            ...
            let excl_focus_on_layer = |layer| {
                layers.layers_on(layer).find_map(|surface| {
                    ...
                    let mapped = self.awm.mapped_layer_surfaces.get(surface)?;
                    if mapped.place_within_backdrop() {
                        return None;
                    }
            ...
            let on_d_focus_on_layer = |layer| {
                layers.layers_on(layer).find_map(|surface| {
                    let is_on_demand_surface =
                        Some(surface) == self.awm.layer_shell_on_demand_focus.as_ref();
        ...
        if self.awm.keyboard_focus != focus {
            trace!(
                "keyboard focus changed from {:?} to {:?}",
                self.awm.keyboard_focus,
                focus
            );
            ...
            {
                if let Some((mapped, _)) = self.awm.layout.find_window_and_output_mut(surface) {
            ...
            } = &focus
            {
                if let Some((mapped, _)) = self.awm.layout.find_window_and_output_mut(surface) {
                    ...
                    let debounce = self.awm.config.borrow().recent_windows.debounce_ms;
                    ...
                    if mapped.get_focus_timestamp().is_none() || debounce.is_zero() {
                        ...
                        let focus_token = self
                            ...
                            .insert_source(timer, move |_, _, state| {
                                state.awm.mru_apply_keyboard_commit();
                                TimeoutAction::Drop
                            })
                            .unwrap();
                        if let Some(PendingMruCommit { token, .. }) =
                            self.awm.pending_mru_commit.replace(PendingMruCommit {
                                id: mapped.id(),
                                token: focus_token,
                                stamp,
                            })
            ...
            if let Some(grab) = self.awm.popup_grab.as_mut() {
                if grab.has_keyboard_grab && Some(&grab.root) != focus.surface() {
                    ...
                    self.awm.seat.get_pointer().unwrap().unset_grab(
                        self,
            ...
            if self.awm.config.borrow().input.keyboard.track_layout == TrackLayout::Window {
                ...
                if let Some(current_focus) = self.awm.keyboard_focus.surface() {
            ...
            self.awm.keyboard_focus.clone_from(&focus);
    ...
    fn load_xkb_file(&mut self) {
        let xkb_file = self.awm.config.borrow().input.keyboard.xkb.file.clone();
    ...
    pub fn reload_config(&mut self, config: Result<Config, ()>) {
        ...
        let mut config = match config {
            Ok(config) => config,
            Err(()) => {
                self.awm.config_error_notification.show();
                self.awm.queue_redraw_all();

                #[cfg(feature = "dbus")]
                self.awm.a11y_announce_config_error();

                return;
            }
        };

        self.awm.config_error_notification.hide();

        // Find & orphan removed named workspaces.
        let mut removed_workspaces: Vec<String> = vec![];
        for ws in &self.awm.config.borrow().workspaces {
            if !config.workspaces.iter().any(|w| w.name == ws.name) {
                removed_workspaces.push(ws.name.0.clone());
            }
        }
        for name in removed_workspaces {
            self.awm.layout.unname_workspace(&name);
        }

        self.awm.layout.update_config(&config);
        for mapped in self.awm.mapped_layer_surfaces.values_mut() {
            mapped.update_config(&config);
        }

        // Create new named workspaces.
        for ws_config in &config.workspaces {
            self.awm.layout.ensure_named_workspace(ws_config);
        }

        let rate = 1.0 / config.animations.slowdown.max(0.001);
        self.awm.clock.set_rate(rate);
        self.awm
            .clock
        ...
        *CHILD_ENV.write().unwrap() = mem::take(&mut config.environment);
        ...
        let mut old_config = self.awm.config.borrow_mut();
        ...
        if config.cursor != old_config.cursor {
            ...
            self.awm.cursor_texture_cache.clear();
        }
        ...
        if config.outputs != self.awm.config_file_output_config {
        ...
        if new_mod_key != self.backend.mod_key(&old_config) || binds_changed {
            ...
            self.awm.mods_with_mouse_binds = mods_with_mouse_binds(new_mod_key, &config.binds);
            self.awm.mods_with_wheel_binds = mods_with_wheel_binds(new_mod_key, &config.binds);
            self.awm.mods_with_tablet_stylus_binds =
                mods_with_tablet_stylus_binds(new_mod_key, &config.binds);
            self.awm.mods_with_finger_scroll_binds =
        ...
        *old_config = config;
        ...
        if let Some(mut xkb) = reload_xkb {
            ...
            if set_xkb_config {
                // If xkb is unset in the awm config, use settings from locale1.
                if xkb == Xkb::default() {
                    trace!("using xkb from locale1");
                    xkb = self.awm.xkb_from_locale1.clone().unwrap_or_default();
                }
        ...
        if libinput_config_changed {
            let config = self.awm.config.borrow();
            for mut device in self.awm.devices.iter().cloned() {
                apply_libinput_settings(&config.input, &mut device);
            }
        }

        if ignored_nodes_changed {
            self.backend.update_ignored_nodes_config(&mut self.awm);
        }
        ...
        if window_rules_changed {
            self.awm.recompute_window_rules();
        }

        if layer_rules_changed {
            self.awm.recompute_layer_rules();
        }

        if shaders_changed {
            self.awm.update_shaders();
        }
        ...
        if xwls_changed {
            ...
            let display_name = (!config.xwayland_satellite.off)
                .then_some(self.awm.satellite.as_ref())
                .flatten()
    ...
    pub fn reload_output_config(&mut self) {
        ...
        for output in self.awm.global_space.outputs() {
            let name = output.user_data().get::<OutputName>().unwrap();
            let full_config = self.awm.config.borrow_mut();
                ...
                || output.current_transform() != transform
            {
                ...
                self.awm.ipc_outputs_changed = true;
            ...
            if let Some(state) = self.awm.output_state.get_mut(output) {
            ...
            for mon in self.awm.layout.monitors_mut() {
        ...
        for output in resized_outputs {
            self.awm.output_resized(&output);
        }
        ...
        self.awm.reposition_outputs(None);

        if let Some(touch) = self.awm.seat.get_touch() {
            touch.cancel(self);
        }

        let config = self.awm.config.borrow().outputs.clone();
        self.awm.output_management_state.on_config_changed(config);
    }

    pub fn modify_output_config<F>(&mut self, name: &str, fun: F)
    where
        F: FnOnce(&mut awm_config::Output),
    {
        ...
        let temp;
        let match_name = if let Some(output) = self.awm.output_by_name_match(name) {
        ...
        let mut config = self.awm.config.borrow_mut();
        let config = if let Some(config) = config.outputs.find_mut(match_name) {
            config
        } else {
            config.outputs.0.push(awm_config::Output {
    ...
    pub fn apply_transient_output_config(&mut self, name: &str, action: awm_ipc::OutputAction) {
        self.modify_output_config(name, move |config| match action {
            awm_ipc::OutputAction::Off => config.off = true,
            awm_ipc::OutputAction::On => config.off = false,
            awm_ipc::OutputAction::Mode { mode } => {
                config.mode = match mode {
                    awm_ipc::ModeToSet::Automatic => None,
                    awm_ipc::ModeToSet::Specific(mode) => Some(awm_config::output::Mode {
                        custom: false,
                        mode,
                    }),
                };
            ...
            awm_ipc::OutputAction::CustomMode { mode } => {
                config.mode = Some(awm_config::output::Mode { custom: true, mode });
                config.modeline = None;
            }
            awm_ipc::OutputAction::Modeline {
                clock,
                hdisplay,
                hsync_start,
                hsync_end,
                ...
                config.modeline = Some(awm_config::output::Modeline {
                    clock,
                    hdisplay,
                    hsync_start,
                    hsync_end,
            ...
            awm_ipc::OutputAction::Scale { scale } => {
                config.scale = match scale {
                    awm_ipc::ScaleToSet::Automatic => None,
                    awm_ipc::ScaleToSet::Specific(scale) => Some(FloatOrInt(scale)),
                }
            }
            awm_ipc::OutputAction::Transform { transform } => config.transform = transform,
            awm_ipc::OutputAction::Position { position } => {
                config.position = match position {
                    awm_ipc::PositionToSet::Automatic => None,
                    awm_ipc::PositionToSet::Specific(position) => Some(awm_config::Position {
                        x: position.x,
                        y: position.y,
                    }),
                }
            }
            awm_ipc::OutputAction::Vrr { vrr } => {
            ...
            awm_ipc::OutputAction::MaxBpc { max_bpc } => config.max_bpc = Some(MaxBpc(max_bpc)),
        });
    ...
    pub fn refresh_ipc_outputs(&mut self) {
        if !self.awm.ipc_outputs_changed {
            return;
        }
        self.awm.ipc_outputs_changed = false;
        ...
        #[cfg(feature = "dbus")]
        self.awm.on_ipc_outputs_changed();

        let new_config = self.backend.ipc_outputs().lock().unwrap().clone();
        self.awm.output_management_state.notify_changes(new_config);
    }

    pub fn open_screenshot_ui(&mut self, show_pointer: bool, path: Option<String>) {
        if self.awm.is_locked() || self.awm.screenshot_ui.is_open() {
            return;
        }
        ...
        let default_output = self
            .awm
            .output_under_cursor()
            .or_else(|| self.awm.layout.active_output().cloned());
        let Some(default_output) = default_output else {
            return;
        };

        self.awm.update_render_elements(None);

        let Some(screenshots) = self
            .backend
            .with_primary_renderer(|renderer| self.awm.capture_screenshots(renderer).collect())
        else {
            return;
        };
        ...
        self.awm.seat.tablet_seat().with_tools(|map| {
    ...
    pub fn handle_pick_color(&mut self, tx: async_channel::Sender<Option<awm_ipc::PickedColor>>) {
        let pointer = self.awm.seat.get_pointer().unwrap();
    ...
    pub fn confirm_screenshot(&mut self, write_to_disk: bool) {
        let ScreenshotUi::Open { path, .. } = &mut self.awm.screenshot_ui else {
            return;
        };
        let path = path.take();

        self.backend.with_primary_renderer(|renderer| {
            match self.awm.screenshot_ui.capture(renderer) {
                Ok((size, pixels)) => {
                    if let Err(err) = self.awm.save_screenshot(size, pixels, write_to_disk, path) {
    ...
    pub fn store_unmap_snapshot(&mut self, window: &Window, output: Option<&Output>) {
        ...
        self.awm.update_xray_render_elements(output);
        ...
        self.backend.with_primary_renderer(|renderer| {
            if let Some(output) = output {
                ...
                self.awm.fill_xray_elements(ctx.r(), output);
                ...
                let has_blocked_out = self.awm.has_blocked_out_background_layers(output);
                if has_blocked_out {
                    let screencast_ctx = RenderCtx {
                        target: RenderTarget::Screencast,
                        ..ctx.r()
                    };
                    self.awm.fill_xray_elements(screencast_ctx, output);
                }

                let state = self.awm.output_state.get_mut(output).unwrap();
                self.awm.layout.store_unmap_snapshot(
                    renderer,
                    Some(&mut state.xray),
                    has_blocked_out,
                    window,
                ...
                self.awm.clear_xray_elements(output);
            } else {
                self.awm
                    .layout
    ...
    pub fn on_screen_shot_msg(
        &mut self,
        to_screenshot: &async_channel::Sender<AWMToScreenshot>,
        msg: ScreenshotToAWM,
    ) {
        match msg {
            ScreenshotToAWM::TakeScreenshot { include_cursor } => {
                self.handle_take_screenshot(to_screenshot, include_cursor);
            }
            ScreenshotToAWM::PickColor(tx) => {
    ...
    fn handle_take_screenshot(
        ...
        let rv = self.backend.with_primary_renderer(|renderer| {
            let on_done = {
                ...
                move |path| {
                    let msg = AWMToScreenshot::ScreenshotResult(Some(path));
            ...
            if let Err(err) = res {
                ...
                let msg = AWMToScreenshot::ScreenshotResult(None);
                if let Err(err) = to_screenshot.send_blocking(msg) {
    ...
    pub fn on_introspect_msg(
        &mut self,
        to_introspect: &async_channel::Sender<AWMToIntrospect>,
        msg: IntrospectToAWM,
    ) {
        use crate::utils::with_toplevel_role;

        let IntrospectToAWM::GetWindows = msg;
        ...
        #[cfg(feature = "xdp-gnome-screencast")]
        windows.insert(
            self.awm.casting.dynamic_cast_id_for_portal.get(),
            gnome_shell_introspect::WindowProperties {
                title: String::from("awm Dynamic Cast Target"),
                app_id: String::from("rs.bxt.awm.desktop"),
            },
        );

        self.awm.layout.with_windows(|mapped, _, _, _| {
        ...
        let msg = AWMToIntrospect::Windows(windows);
    ...
    #[cfg(feature = "dbus")]
    pub fn on_login1_msg(&mut self, msg: Login1ToAWM) {
        let Login1ToAWM::LidClosedChanged(is_closed) = msg;
    ...
    #[cfg(feature = "dbus")]
    pub fn on_locale1_msg(&mut self, msg: Locale1ToAWM) {
        let Locale1ToAWM::XkbChanged(xkb) = msg;

        trace!("locale1 xkb settings changed: {xkb:?}");
        let xkb = self.awm.xkb_from_locale1.insert(xkb);

        {
            let config = self.awm.config.borrow();
            if config.input.keyboard.xkb != Xkb::default() {
                trace!("ignoring locale1 xkb change because awm config has xkb settings");
                return;
            }
        }
...
impl AWM {
    pub fn new(
        ...
        let _span = tracy_client::span!("AWM::new");
        ...
        event_loop
            .insert_source(
                ...
                |_, _, state| {
                    state.awm.activation_state.retain_tokens(|_, token_data| {
        ...
        event_loop
            .insert_source(
                ...
                |_, _, state| {
                    state.awm.send_frame_callbacks_on_fallback_timer();
        ...
        let socket_name = create_wayland_socket.then(|| {
            ...
            event_loop
                .insert_source(socket_source, move |client, _, state| {
                    state.awm.insert_client(NewClient {
                        client,
                        restricted: false,
        ...
        event_loop
            .insert_source(
                ...
                |_, _, state| {
                    ...
                    state.awm.is_at_startup = false;
                    state.awm.recompute_window_rules();
                    state.awm.recompute_layer_rules();
                    TimeoutAction::Drop
                },
            )
            .unwrap();
        ...
        awm.reset_pointer_inactivity_timer();
    ...
    pub fn inhibit_power_key(&mut self) -> anyhow::Result<()> {
        ...
        let message = conn.call_method(
            ...
            "Inhibit",
            &("handle-power-key", "awm", "Power key handling", "block"),
        )?;
    ...
    pub fn reposition_outputs(&mut self, new_output: Option<&Output>) {
        let _span = tracy_client::span!("AWM::reposition_outputs");
        ...
        struct Data {
            ...
            config: Option<awm_config::Position>,
        }
    ...
    /// exists in awm. Right after the output global is disabled, but before it is removed for
    /// good, `Output::from_resource` will succeed, but since awm already forgot the output,
    ...
    pub fn redraw_queued_outputs(&mut self, backend: &mut Backend) {
        let _span = tracy_client::span!("AWM::redraw_queued_outputs");
    ...
    pub fn render_pointer<R: AWMRenderer>(
        &self,
        renderer: &mut R,
        output: &Output,
        push: &mut dyn FnMut(PointerRenderElements<R>),
    ) {
        let _span = tracy_client::span!("AWM::render_pointer");
    ...
    pub fn refresh_pointer_outputs(&mut self) {
        ...
        let _span = tracy_client::span!("AWM::refresh_pointer_outputs");
    ...
    pub fn refresh_idle_inhibit(&mut self) {
        let _span = tracy_client::span!("AWM::refresh_idle_inhibit");
    ...
    pub fn refresh_window_states(&mut self) {
        let _span = tracy_client::span!("AWM::refresh_window_states");
    ...
    pub fn refresh_window_rules(&mut self) {
        let _span = tracy_client::span!("AWM::refresh_window_rules");
    ...
    pub fn advance_animations(&mut self) {
        let _span = tracy_client::span!("AWM::advance_animations");
    ...
    pub fn render_to_vec<R: AWMRenderer>(
        &self,
        ctx: RenderCtx<R>,
        output: &Output,
    ...
    pub fn render<R: AWMRenderer>(
        ...
        let _span = tracy_client::span!("AWM::render");
    ...
    pub fn fill_xray_elements(&self, mut ctx: RenderCtx<GlesRenderer>, output: &Output) {
        let _span = tracy_client::span!("AWM::fill_xray_elements");
    ...
    fn render_layer_normal<R: AWMRenderer>(
        &self,
        mut ctx: RenderCtx<R>,
        ns: Option<usize>,
        layer_map: &LayerMap,
    ...
    #[allow(clippy::too_many_arguments)]
    fn render_layer_popups<R: AWMRenderer>(
        &self,
        mut ctx: RenderCtx<R>,
        ns: Option<usize>,
        layer_map: &LayerMap,
    ...
    fn redraw(&mut self, backend: &mut Backend, output: &Output) {
        let _span = tracy_client::span!("AWM::redraw");
    ...
    pub fn refresh_on_demand_vrr(&mut self, backend: &mut Backend, output: &Output) {
        let _span = tracy_client::span!("AWM::refresh_on_demand_vrr");
    ...
    pub fn update_primary_scanout_output(
        ...
        // as in awm they can only be rendered on a single output at a time.
        //
    ...
    pub fn send_dmabuf_feedbacks(
        ...
        let _span = tracy_client::span!("AWM::send_dmabuf_feedbacks");
    ...
    pub fn send_frame_callbacks(&mut self, output: &Output) {
        let _span = tracy_client::span!("AWM::send_frame_callbacks");
    ...
    pub fn send_frame_callbacks_on_fallback_timer(&mut self) {
        let _span = tracy_client::span!("AWM::send_frame_callbacks_on_fallback_timer");
    ...
    pub fn render_for_screencopy_with_damage(
        ...
        let _span = tracy_client::span!("AWM::render_for_screencopy_with_damage");
    ...
    pub fn render_for_screencopy_without_damage(
        ...
        let _span = tracy_client::span!("AWM::render_for_screencopy");
    ...
    pub fn screenshot(
        ...
        let _span = tracy_client::span!("AWM::screenshot");
    ...
    pub fn screenshot_window(
        ...
        let _span = tracy_client::span!("AWM::screenshot_window");
    ...
    pub fn screenshot_all_outputs(
        ...
        let _span = tracy_client::span!("AWM::screenshot_all_outputs");
    ...
    pub fn on_ipc_outputs_changed(&self) {
        let _span = tracy_client::span!("AWM::on_ipc_outputs_changed");
    ...
    pub fn do_screen_transition(&mut self, renderer: &mut GlesRenderer, delay_ms: Option<u16>) {
        let _span = tracy_client::span!("AWM::do_screen_transition");
    ...
    pub fn recompute_window_rules(&mut self) {
        let _span = tracy_client::span!("AWM::recompute_window_rules");
    ...
    pub fn recompute_layer_rules(&mut self) {
        let _span = tracy_client::span!("AWM::recompute_layer_rules");
    ...
    pub fn reset_pointer_inactivity_timer(&mut self) {
        ...
        let _span = tracy_client::span!("AWM::reset_pointer_inactivity_timer");
        ...
        let token = self
            .event_loop
            .insert_source(timer, move |_, _, state| {
                state.awm.pointer_inactivity_timer = None;

                // If the pointer is already invisible, don't reset it back to Hidden causing one
                // frame of hover.
                if state.awm.pointer_visibility.is_visible() {
                    state.awm.pointer_visibility = PointerVisibility::Hidden;
                    state.awm.queue_redraw_all();
                }
    ...
    pub fn notify_activity(&mut self) {
        ...
        let _span = tracy_client::span!("AWM::notify_activity");
