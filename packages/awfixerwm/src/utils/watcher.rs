...
use awm_config::{Config, ConfigParseResult, ConfigPath};
...
pub fn setup(state: &mut State, config_path: &ConfigPath, includes: Vec<PathBuf>) {
    ...
    state.awm.config_file_watcher = Some(watcher);
}
...
mod tests {
    ...
    fn change_file() -> Result {
        TestPath::Explicit("awm/config.kdl")
            .setup(|sh| sh.write_file("awm/config.kdl", "a"))
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("awm/config.kdl", "b")?;
    ...
    fn overwrite_but_dont_change_file() -> Result {
        TestPath::Explicit("awm/config.kdl")
            ...
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("awm/config.kdl", "a")?;
    ...
    fn touch_file() -> Result {
        TestPath::Explicit("awm/config.kdl")
            ...
            .assert_initial("a")
            .run(|sh, test| {
                cmd!(sh, "touch awm/config.kdl").run()?;
                test.assert_changed_to("a");
    ...
    fn change_linked_file() -> Result {
        TestPath::Explicit("awm/config.kdl")
            .setup(|sh| {
                sh.write_file("awm/config2.kdl", "a")?;
                cmd!(sh, "ln -sf config2.kdl awm/config.kdl").run()
            })
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("awm/config2.kdl", "b")?;
                test.assert_changed_to("b");
    ...
    fn change_file_in_linked_dir() -> Result {
        TestPath::Explicit("awm/config.kdl")
            .setup(|sh| {
                sh.write_file("awm2/config.kdl", "a")?;
                cmd!(sh, "ln -s awm2 awm").run()
            })
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("awm2/config.kdl", "b")?;
                test.assert_changed_to("b");
    ...
    fn swap_dir_link() -> Result {
        TestPath::Explicit("awm/config.kdl")
            ...
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("awm3/config.kdl", "b")?;
                sh.remove_path("awm")?;
                cmd!(sh, "ln -s awm3 awm").run()?;
                test.assert_changed_to("b");
    ...
    fn change_included_file() -> Result {
        TestPath::Explicit("awm/config.kdl")
            .setup(|sh| {
                sh.write_file("awm/config.kdl", "include \"colors.kdl\"")?;
                sh.write_file("awm/colors.kdl", "// Colors")
            })
            .assert_initial("include \"colors.kdl\"")
            .run(|sh, test| {
                sh.write_file("awm/colors.kdl", "// Updated colors")?;
    ...
    fn nested_includes() -> Result {
        TestPath::Explicit("awm/config.kdl")
            .setup(|sh| {
                sh.write_file("awm/config.kdl", "include \"a.kdl\"")?;
                sh.write_file("awm/a.kdl", "include \"b.kdl\"")?;
                sh.write_file("awm/b.kdl", "// b content")
            })
            .assert_initial("include \"a.kdl\"")
            .run(|sh, test| {
                sh.write_file("awm/b.kdl", "// updated b")?;
    ...
    fn broken_include_still_gets_watched() -> Result {
        TestPath::Explicit("awm/config.kdl")
            .setup(|sh| {
                sh.write_file("awm/config.kdl", "include \"colors.kdl\"")?;
                sh.write_file("awm/colors.kdl", "broken")
            })
            .assert_initial("include \"colors.kdl\"")
            .run(|sh, test| {
                sh.write_file("awm/colors.kdl", "// Fixed")?;
                test.assert_changed_to("include \"colors.kdl\"");
    ...
    fn swap_just_link() -> Result {
        TestPath::Explicit("awm/config.kdl")
            .setup_any(|sh| {
                let dir = sh.current_dir().join("awm");
                ...
                cmd!(sh, "ln -s config2.kdl awm/config.kdl").run()?;

                Ok(())
            })
            .assert_initial("a")
            .run(|sh, test| {
                cmd!(sh, "ln -sf config3.kdl awm/config.kdl").run()?;
                test.assert_changed_to("b");
    ...
    fn swap_many_regular() -> Result {
        TestPath::Regular {
            user_path: "user-awm/config.kdl",
            system_path: "system-awm/config.kdl",
        }
        .setup(|sh| sh.write_file("system-awm/config.kdl", "system config"))
        .assert_initial("system config")
        .run(|sh, test| {
            sh.write_file("user-awm/config.kdl", "user config")?;
            test.assert_changed_to("user config");

            cmd!(sh, "touch system-awm/config.kdl").run()?;
            test.assert_unchanged();

            sh.remove_path("system-awm")?;
            test.assert_unchanged();

            sh.write_file("system-awm/config.kdl", "new system config")?;
            test.assert_unchanged();

            sh.remove_path("user-awm")?;
            test.assert_changed_to("new system config");

            sh.write_file("system-awm/config.kdl", "updated system config")?;
            test.assert_changed_to("updated system config");

            sh.write_file("user-awm/config.kdl", "new user config")?;
    ...
    fn swap_many_links_regular_like_nix() -> Result {
        ...
        .setup_any(|sh| {
            ...
            sh.create_dir("user-awm")?;
            sh.create_dir("system-awm")?;
        ...
        .run(|sh, test| {
            ...
            cmd!(sh, "ln -s {store}/gen1 user-awm/config.kdl").run()?;
            test.assert_changed_to("gen 1");

            cmd!(sh, "ln -s {store}/gen2 system-awm/config.kdl").run()?;
            test.assert_unchanged();

            cmd!(sh, "unlink user-awm/config.kdl").run()?;
            test.assert_changed_to("gen 2");

            cmd!(sh, "ln -s {store}/gen3 user-awm/config.kdl").run()?;
            test.assert_changed_to("gen 3");

            cmd!(sh, "ln -sf {store}/gen1 system-awm/config.kdl").run()?;
            test.assert_unchanged();

            cmd!(sh, "unlink system-awm/config.kdl").run()?;
            test.assert_unchanged();

            cmd!(sh, "ln -s {store}/gen1 system-awm/config.kdl").run()?;
            test.assert_unchanged();
