...
mod systemd {
    ...
    pub fn do_spawn(command: &OsStr, mut process: Command) -> Option<Child> {
        ...
        // scopes in order to separate them from the awm process. This is helpful for
        // example to prevent the OOM killer from taking down awm together with a
        // misbehaving client.
        //
        ...
        // intermediate child we will send back the grandchild PID, and in awm we will create a
    ...
    fn start_systemd_scope(
        ...
        let mut scope_name = String::from("app-awm-");
