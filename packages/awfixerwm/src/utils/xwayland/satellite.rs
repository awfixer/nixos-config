...
pub fn setup(state: &mut State) {
    if state.awm.satellite.is_some() {
        return;
    }

    let config = state.awm.config.borrow();
    ...
    let event_loop = &state.awm.event_loop;
    ...
    state.awm.satellite = Some(Satellite {
        x11,
        abstract_token: None,
        unix_token: None,
        to_main,
...
fn setup_watch(state: &mut State) {
    let Some(satellite) = state.awm.satellite.as_mut() else {
        return;
    };
    ...
    if let Some(fd) = &satellite.x11.abstract_fd {
        ...
        let token = event_loop
            .insert_source(source, move |_, _, state| {
                if let Some(satellite) = &mut state.awm.satellite {
                    // Remove the other source.
                    if let Some(token) = satellite.unix_token.take() {
                        state.awm.event_loop.remove(token);
                    }
                    // Clear this source.
                    satellite.abstract_token = None;

                    debug!("connection to X11 abstract socket; spawning xwayland-satellite");
                    let path = state.awm.config.borrow().xwayland_satellite.path.clone();
...
fn spawn(path: String, xwl: &Satellite) {
    ...
    // We don't want it spamming the awm output.
    process
        .env_remove("RUST_BACKTRACE")
        .env_remove("RUST_LIB_BACKTRACE");
    process
