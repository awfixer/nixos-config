...
void main() {
    vec3 coords_geo = input_to_geo * vec3(awm_v_coords, 1.0);
    vec3 coords_window_geo = window_input_to_geo * vec3(awm_v_coords, 1.0);
    ...
    if (sigma < 0.1) {
        ...
        shadow_value = awm_rounding_alpha(coords_geo.xy, geo_size, corner_radius);
    } else {
    ...
    if (window_geo_size != vec2(0.0, 0.0)) {
        if (0.0 <= coords_window_geo.x && coords_window_geo.x <= window_geo_size.x
            ...
            float alpha = awm_rounding_alpha(coords_window_geo.xy, window_geo_size, window_corner_radius);
