...
use super::renderer::{AsGlesFrame as _, AWMRenderer};
use super::shaders::{mat3_uniform, Shaders};
use crate::backend::tty::{TtyFrame, TtyRenderer, TtyRendererError};

#[derive(Debug)]
pub struct ClippedSurfaceRenderElement<R: AWMRenderer> {
...
impl<R: AWMRenderer> ClippedSurfaceRenderElement<R> {
    pub fn new(
    ...
    fn compute_uniforms(&self) -> Vec<Uniform<'static>> {
        ...
        vec![
            Uniform::new("awm_scale", self.scale),
...
impl<R: AWMRenderer> Element for ClippedSurfaceRenderElement<R> {
    fn id(&self) -> &Id {
        self.inner.id()
    }
