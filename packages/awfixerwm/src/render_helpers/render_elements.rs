...
#[macro_export]
macro_rules! awm_render_elements {
    ...
    ($name:ident<R> => { $($variant:ident = $type:ty),+ $(,)? }) => {
        $crate::awm_render_elements!(@impl $name () ($name<R>) => { $($variant = $type),+ });

        $(impl<R: $crate::render_helpers::renderer::AWMRenderer> From<$type> for $name<R> {
            fn from(x: $type) -> Self {
                Self::$variant(x)
            }
        })+
    ...
    ($name:ident => { $($variant:ident = $type:ty),+ $(,)? }) => {
        $crate::awm_render_elements!(@impl $name ($name) () => { $($variant = $type),+ });
    ...
    (@impl $name:ident ($($name_no_R:ident)?) ($($name_R:ident<$R:ident>)?) => { $($variant:ident = $type:ty),+ }) => {
        ...
        #[derive(Debug)]
        pub enum $name$(<$R: $crate::render_helpers::renderer::AWMRenderer>)? {
            $($variant($type)),+
        }

        impl$(<$R: $crate::render_helpers::renderer::AWMRenderer>)? smithay::backend::renderer::element::Element for $name$(<$R>)? {
