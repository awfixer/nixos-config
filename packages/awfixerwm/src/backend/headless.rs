...
impl Headless {
    ...
    pub fn init(&mut self, _awm: &mut AWM) {}
    ...
    pub fn add_output(&mut self, awm: &mut AWM, n: u8, size: (u16, u16)) {
        let connector = format!("headless-{n}");
        let make = "awm".to_string();
        ...
        awm.add_output(output, None, false);
    }
    ...
    pub fn render(&mut self, awm: &mut AWM, output: &Output) -> RenderResult {
        ...
        let mut presentation_feedbacks = awm.take_presentation_feedbacks(output, &states);
