...
pub type IpcOutputMap = HashMap<OutputId, awm_ipc::Output>;
...
impl Backend {
    pub fn init(&mut self, awm: &mut AWM) {
        ...
        match self {
            Backend::Tty(tty) => tty.init(awm),
            Backend::Winit(winit) => winit.init(awm),
            Backend::Headless(headless) => headless.init(awm),
        }
    }
    ...
    pub fn render(
        ...
        match self {
            Backend::Tty(tty) => tty.render(awm, output, target_presentation_time),
            Backend::Winit(winit) => winit.render(awm, output),
            Backend::Headless(headless) => headless.render(awm, output),
        }
    }
    ...
    pub fn set_output_on_demand_vrr(&mut self, awm: &mut AWM, output: &Output, enable_vrr: bool) {
        match self {
            Backend::Tty(tty) => tty.set_output_on_demand_vrr(awm, output, enable_vrr),
            Backend::Winit(_) => (),
            Backend::Headless(_) => (),
        }
    }
    ...
    pub fn update_ignored_nodes_config(&mut self, awm: &mut AWM) {
        match self {
            Backend::Tty(tty) => tty.update_ignored_nodes_config(awm),
            Backend::Winit(_) => (),
            Backend::Headless(_) => (),
        }
    }
    ...
    pub fn on_output_config_changed(&mut self, awm: &mut AWM) {
        match self {
            Backend::Tty(tty) => tty.on_output_config_changed(awm),
            Backend::Winit(_) => (),
            Backend::Headless(_) => (),
        }
    }
