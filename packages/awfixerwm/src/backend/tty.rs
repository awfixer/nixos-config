...
use libc::dev_t;
use awm_config::output::{MaxBpc, Modeline};
use awm_config::{Config, OutputName};
use awm_ipc::{HSyncPolarity, VSyncPolarity};
...
use crate::awm::{AWM, RedrawState, State};
...
impl OutputDevice {
    ...
    fn cleanup_mismatching_resources(
        ...
        for (conn, info) in self.drm_scanner.connectors() {
            // We only keep the connector if it has a CRTC and the output isn't off in awm.
...
impl Tty {
    pub fn new(
        ...
        let (session, notifier) = LibSeatSession::new().context(
            "Error creating a session. This might mean that you're trying to run awm on a TTY \
        ...
        let udev_dispatcher = Dispatcher::new(udev_backend, move |event, _, state: &mut State| {
            state.backend.tty().on_udev_event(&mut state.awm, event);
        });
        event_loop
        ...
        // If the session is not active at startup (e.g. awm was launched from a different TTY),
        ...
        event_loop
            .insert_source(notifier, move |event, _, state| {
                state.backend.tty().on_session_event(&mut state.awm, event);
            })
            .unwrap();
    ...
    pub fn init(&mut self, awm: &mut AWM) {
        ...
        //
        // This can happen when starting awm while having a different TTY active (e.g. via tmux).
        ...
        {
            if let Err(err) = self.device_added(primary_device_id, primary_device_path, awm) {
                warn!(
        ...
        for (device_id, path) in udev.device_list() {
            ...
            if let Err(err) = self.device_added(device_id, path, awm) {
                warn!("error adding device: {err:?}");
            }
        }
    }

    fn on_udev_event(&mut self, awm: &mut AWM, event: UdevEvent) {
        ...
        match event {
            ...
            UdevEvent::Changed { device_id } => {
                ...
                self.device_changed(device_id, awm, false)
            }
            UdevEvent::Removed { device_id } => {
                ...
                self.device_removed(device_id, awm)
            }
        }
    }

    fn on_session_event(&mut self, awm: &mut AWM, event: SessionEvent) {
        ...
        match event {
            ...
            SessionEvent::ActivateSession => {
                ...
                for node in removed_devices {
                    ...
                    self.device_removed(node.dev_id(), awm);
                }
                ...
                for node in remained_devices {
                    ...
                    // https://github.com/awfixerwm/awm/issues/3048
                    ...
                    self.device_changed(node.dev_id(), awm, true);
                ...
                if self.update_output_config_on_resume {
                    self.on_output_config_changed(awm);
                }

                self.refresh_ipc_outputs(awm);

                awm.notify_activity();
                awm.monitors_active = true;
                self.set_monitors_active(true);
                awm.queue_redraw_all();
            }
        }
    }
    ...
    fn device_added(
        ...
        if render_node == Some(self.primary_render_node) && self.dmabuf_global.is_none() {
            ...
            if let Err(err) = renderer.bind_wl_display(&awm.display_handle) {
            ...
            .insert_source(drm_notifier, move |event, meta, state| {
                ...
                match event {
                    DrmEvent::VBlank(crtc) => {
                        ...
                        tty.on_vblank(&mut state.awm, node, crtc, meta);
                    }
        ...
        let drm_lease_state = DrmLeaseState::new::<State>(&awm.display_handle, &node)
        ...
        self.device_changed(device_id, awm, true);

        Ok(())
    }

    fn device_changed(&mut self, device_id: dev_t, awm: &mut AWM, cleanup: bool) {
        ...
        for crtc in &removed {
            self.connector_disconnected(awm, node, *crtc);
        }
        ...
        if cleanup {
            ...
            let disable_laptop_panels = self.should_disable_laptop_panels(awm.is_lid_closed);
    ...
    fn device_removed(&mut self, device_id: dev_t, awm: &mut AWM) {
        ...
        if let Some(render_node) = device.render_node {
            ...
            if was_last && render_node == self.primary_render_node {
                ...
                if let Some(global) = self.dmabuf_global.take() {
                    awm.dmabuf_state
                        .disable_global::<State>(&awm.display_handle, &global);
                    awm.event_loop
                        .insert_source(
                            ...
                            move |_, _, state| {
                                state
                                    .awm
                                    .dmabuf_state
                                    .destroy_global::<State>(&state.awm.display_handle, global);
                                TimeoutAction::Drop
                            },
                        )
                        .unwrap();
        ...
        awm.event_loop.remove(device.token);
    ...
    fn connector_connected(
        ...
        awm.add_output(output.clone(), Some(refresh_interval(mode)), vrr_enabled);

        if awm.monitors_active {
            // Redraw the new monitor.
            awm.event_loop.insert_idle(move |state| {
                // Guard against output disconnecting before the idle has a chance to run.
                if state.awm.output_state.contains_key(&output) {
                    state.awm.queue_redraw(&output);
                }
            });
        }
    ...
    fn connector_disconnected(&mut self, awm: &mut AWM, node: DrmNode, crtc: crtc::Handle) {
        let Some(device) = self.devices.get_mut(&node) else {
    ...
    fn on_vblank(
        ...
        let presentation_time = if awm.config.borrow().debug.emulate_zero_presentation_time {
            Duration::ZERO
        } else {
            presentation_time
        };
        ...
        let Some(output_state) = awm.output_state.get_mut(&output) else {
        ...
        let redraw_needed = match mem::replace(&mut output_state.redraw_state, RedrawState::Idle) {
            ...
            | RedrawState::WaitingForEstimatedVBlankAndQueued(_)) => {
                ...
                //
                // https://github.com/awfixerwm/awm/issues/556
                // https://github.com/awfixerwm/awm/issues/615
                error!(
        ...
        if redraw_needed || output_state.unfinished_animations_remain {
            ...
            awm.queue_redraw(&output);
        } else {
            awm.send_frame_callbacks(&output);
        }
    }

    fn on_estimated_vblank_timer(&self, awm: &mut AWM, output: Output) {
    ...
    pub fn render(
        ...
        let mut elements = awm.render_to_vec(ctx, output, true);

        // Visualize the damage, if enabled.
        if awm.debug_draw_damage {
            let output_state = awm.output_state.get_mut(output).unwrap();
        ...
        let flags = {
            ...
            if debug.skip_cursor_only_updates_during_vrr {
                let output_state = awm.output_state.get(output).unwrap();
                if output_state.frame_clock.vrr() {
        ...
        match drm_compositor.render_frame::<_, _>(&mut renderer, &elements, [0.; 4], flags) {
            Ok(res) => {
                ...
                awm.update_primary_scanout_output(output, &res.states);
                if let Some(dmabuf_feedback) = surface.dmabuf_feedback.as_ref() {
                    awm.send_dmabuf_feedbacks(output, dmabuf_feedback, &res.states);
                }

                if !res.is_empty {
                    let presentation_feedbacks =
                        awm.take_presentation_feedbacks(output, &res.states);
        ...
        queue_estimated_vblank_timer(awm, output.clone(), target_presentation_time);
    ...
    fn refresh_ipc_outputs(&self, awm: &mut AWM) {
        ...
        for (node, device) in &self.devices {
            for (connector, crtc) in device.drm_scanner.crtcs() {
                ...
                let mut modes: Vec<awm_ipc::Mode> = connector
                    .modes()
                    .iter()
    ...
    pub fn set_output_on_demand_vrr(&mut self, awm: &mut AWM, output: &Output, enable_vrr: bool) {
    ...
    pub fn update_ignored_nodes_config(&mut self, awm: &mut AWM) {
    ...
    pub fn on_output_config_changed(&mut self, awm: &mut AWM) {
        ...
        for (node, connector, crtc, _name) in to_connect {
            if let Err(err) = self.connector_connected(awm, node, connector, crtc) {
...
fn queue_estimated_vblank_timer(
    ...
    let token = awm
        ...
        .insert_source(timer, move |_, _, data| {
            data.backend
                .tty()
                .on_estimated_vblank_timer(&mut data.awm, output.clone());
            TimeoutAction::Drop
        })
        .unwrap();
    output_state.redraw_state = RedrawState::WaitingForEstimatedVBlank(token);
...
fn pick_mode(
    connector: &connector::Info,
    target: Option<awm_config::output::Mode>,
...
mod tests {
    ...
    use awm_config::output::Modeline;
    use awm_ipc::{HSyncPolarity, VSyncPolarity};
