...
impl Winit {
    pub fn new(
        ...
        let builder = WindowAttributes::default()
            ...
            .with_platform_attributes(Box::new(
                WindowAttributesWayland::default().with_name("awm", ""),
            ));
            ...
            .insert_source(winit, move |event, _, state| match event {
                WinitEvent::Resized { size, .. } => {
...

                    {
                        ...
                        state.awm.ipc_outputs_changed = true;
                    }

                    state.awm.output_resized(&winit.output);
                }
                WinitEvent::Input(event) => state.process_input_event(event),
                WinitEvent::Focus(_) => (),
                WinitEvent::Redraw => state.awm.queue_redraw(&state.backend.winit().output),
                WinitEvent::CloseRequested => state.awm.stop_signal.stop(),
            })
            .unwrap();
    ...
    pub fn init(&mut self, awm: &mut AWM) {
        ...
        self.create_dmabuf_global(awm);

        awm.add_output(self.output.clone(), None, false);
    }

    pub fn create_dmabuf_global(&mut self, awm: &mut AWM) {
        let renderer = self.backend.renderer();
        ...
        let dmabuf_global = match default_feedback() {
            Ok(feedback) => awm
                .dmabuf_state
                .create_global_with_default_feedback::<State>(&awm.display_handle, &feedback),
            Err(err) => {
                debug!("failed building default dmabuf feedback, falling back to v3: {err:?}");
                let primary_formats = renderer.dmabuf_formats();
                awm.dmabuf_state
                    .create_global::<State>(&awm.display_handle, primary_formats)
            }
        };
    ...
    pub fn render(&mut self, awm: &mut AWM, output: &Output) -> RenderResult {
        ...
        if let Some(damage) = res.damage {
            ...
            let mut presentation_feedbacks = awm.take_presentation_feedbacks(output, &res.states);
