This is a checklist of things to release a new awm version.
...
- Dependency example in `awm-ipc/README.md`
- Dependency example in `awm-ipc/src/lib.rs`
...
- Run `cargo package -p awm-ipc` and make sure it succeeds.
- Make sure the CI passes.
- Make sure the awm-git COPR build passes.
...
- build and test awm with that dependency archive,
...
## Update the awm COPR spec, update licenses in .spec.rpkg

You can grab the previous spec from [the last build](https://copr.fedorainfracloud.org/coprs/yalter/awm/builds/) in the COPR.
...
Update it in both the COPR spec and in `awm.spec.rpkg` accordingly.

If you had to update `awm.spec.rpkg` and therefore make another commit to the awm repo, make sure to update the commit hash in the COPR spec again.
...
## Publish the awm-ipc crate

```
cargo publish -p awm-ipc
```
...
- Generate data:
    ...
    ```
    wlprobe > ./src/data/compositors/awm.json
    ```
