...
By default, awm will attempt to turn on all connected monitors using their preferred modes.
...
You can find all of these by running `awm msg outputs`.
...
If the refresh rate is omitted, awm will pick the highest refresh rate for the resolution.

If the mode is omitted altogether or doesn't work, awm will try to pick one automatically.

Run `awm msg outputs` while inside a awm instance to list all outputs and their modes.
The refresh rate that you set here must match *exactly*, down to the three decimal digits, to what you see in `awm msg outputs`.
...
AWM is asking the monitor to run in a mode that is not supported by the manufacturer.
...
AWM is asking the monitor to run in a mode not supported by the manufacturer.
Use at your own risk.
...
<sup>Since: 0.1.6</sup> If scale is unset, awm will guess an appropriate scale based on the physical dimensions and the resolution of the monitor.
...
AWM repositions outputs from scratch every time the output configuration changes (which includes monitors disconnecting and connecting).
...
1. Try to place every output with explicitly configured `position`, in order. If the output overlaps previously placed outputs, place it to the right of all previously placed outputs. In this case, awm will also print a warning.
...
You can check whether an output supports VRR in `awm msg outputs`.
...
Focus this output by default when awm starts.

If multiple outputs with `focus-at-startup` are connected, they are prioritized in the order that they appear in the config.

When none of the connected outputs are explicitly `focus-at-startup`, awm will focus the first one sorted by name (same output sorting as used elsewhere in awm).
...
Set the background color that awm draws for workspaces on this output.
...
Set the backdrop color that awm draws for this output.
