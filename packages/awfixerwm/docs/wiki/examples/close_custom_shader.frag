...
vec4 solid_gradient(vec3 coords_geo, vec3 size_geo) {
    ...
    // Make it transparent.
    color *= (1.0 - awm_clamped_progress);
...
vec4 default_close(vec3 coords_geo, vec3 size_geo) {
    ...
    float scale = max(0.0, ((1.0 - awm_clamped_progress) / 5.0 + 0.8));
    coords_geo = vec3((coords_geo.xy - vec2(0.5)) / scale + vec2(0.5), 1.0);

    // Get color from the window texture.
    vec3 coords_tex = awm_geo_to_tex * coords_geo;
    vec4 color = texture2D(awm_tex, coords_tex.st);
...
vec4 fall_and_rotate(vec3 coords_geo, vec3 size_geo) {
    ...
    float progress = awm_clamped_progress * awm_clamped_progress;
    ...
    float random = (awm_random_seed - 0.5) / 2.0;
