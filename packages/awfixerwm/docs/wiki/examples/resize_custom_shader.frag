...
//
// It should not contain any uniform definitions or anything else, as awm
// provides them for you.
//
// All symbols defined by awm will have a awm_ prefix, so don't use it for
...
// The pixel color will be further processed by awm (for example, to apply the
// final opacity from window rules).

// Now let's go over the uniforms that awm defines.
//
...
vec4 stretch_next(vec3 coords_curr_geo, vec3 size_curr_geo) {
    vec3 coords_tex_next = awm_geo_to_tex_next * coords_curr_geo;
    vec4 color = texture2D(awm_tex_next, coords_tex_next.st);
    return color;
}
...
vec4 stretch_or_crop_next(vec3 coords_curr_geo, vec3 size_curr_geo) {
    vec3 coords_next_geo = awm_curr_geo_to_next_geo * coords_curr_geo;

    vec3 coords_stretch = awm_geo_to_tex_next * coords_curr_geo;
    vec3 coords_crop = awm_geo_to_tex_next * coords_next_geo;

    // We can crop if the current window size is smaller than the next window
    // size. One way to tell is by comparing to 1.0 the X and Y scaling
    // coefficients in the current-to-next transformation matrix.
    bool can_crop_by_x = awm_curr_geo_to_next_geo[0][0] <= 1.0;
    bool can_crop_by_y = awm_curr_geo_to_next_geo[1][1] <= 1.0;
    ...
    vec4 color = texture2D(awm_tex_next, coords.st);
...
vec4 crossfade_or_crop_next(vec3 coords_curr_geo, vec3 size_curr_geo) {
    vec3 coords_next_geo = awm_curr_geo_to_next_geo * coords_curr_geo;
    vec3 coords_prev_geo = awm_curr_geo_to_prev_geo * coords_curr_geo;

    vec3 coords_crop = awm_geo_to_tex_next * coords_next_geo;
    vec3 coords_stretch = awm_geo_to_tex_next * coords_curr_geo;
    vec3 coords_stretch_prev = awm_geo_to_tex_prev * coords_curr_geo;
    ...
    if (crop) {
        ...
        if (coords_curr_geo.x < 0.0 || 1.0 < coords_curr_geo.x ||
            ...
            color = texture2D(awm_tex_next, coords_crop.st);
        }
    } else {
        // If we can't crop, then crossfade.
        color = texture2D(awm_tex_next, coords_stretch.st);
        vec4 color_prev = texture2D(awm_tex_prev, coords_stretch_prev.st);
        color = mix(color_prev, color, awm_clamped_progress);
    }
