...
AWM will generally start drawing the next frame as soon as the previous one shows up on screen.
...
The solution in awm is a `LazyClock`, a clock that remembers one timestamp.
...
In awm, the timestamp is cleared at the end of every event loop iteration, right before going to sleep waiting for new events.
...
The clock is shared among all animations in awm through passing around and storing a reference-counted pointer.
