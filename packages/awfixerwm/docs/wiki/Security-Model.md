AWM assumes that programs running unsandboxed on the host are **trusted**.

This is a reasonable assumption because programs running on the host have a wide variety of ways to get all access they need, even without awm.
For instance:
...
Anything with access to awm's Wayland socket can, among other things:
...
- Kill a running lockscreen, create a new lock surface, and tell awm to unlock a locked session.

Anything with access to awm's [IPC](./IPC.md) socket can, among other things:

- Spawn a Wayland client which can do everything in the list above.

Anything with access to awm's D-Bus interfaces can, among other things:
...
Importantly, the **quit** action is allowed—you can always quit awm, even when on a lock screen.
Therefore, you must ensure that quitting awm does not drop you into an unprotected TTY commandline.
Usually, a display manager, like GDM, will do this for you: when awm exits (via the quit bind or if it crashes), it'll put you back into a safe password prompt.

Other than quitting, the only way to exit a lock screen is for the lock screen client to tell awm to unlock the session.
