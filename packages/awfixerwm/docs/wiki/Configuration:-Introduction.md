...
You can run `awm validate` to parse the config and see any errors.

To use a different config file path, pass it in the `--config` or `-c` argument to `awm`.

You can also set `$AWM_CONFIG` to the path of the config file.
`--config` always takes precedence.
If `--config` or `$AWM_CONFIG` doesn't point to a real file, the config will not be loaded.
If `$AWM_CONFIG` is set to an empty string, it is ignored and the default config location is used instead.
...
Toggle options in awm are commonly represented as flags.
...
As a rule, awm updates should not break existing config files.
(For example, the default config from awm v0.1.0 still parses fine on v25.02 as I'm writing this.)

Exceptions can be made for parsing bugs.
For example, awm used to accept multiple binds to the same key, but this was not intended and did not do anything (the first bind was always used).
A patch release changed awm from silently accepting this to causing a parsing failure.
...
Keep in mind that the breaking change policy applies only to awm releases.
