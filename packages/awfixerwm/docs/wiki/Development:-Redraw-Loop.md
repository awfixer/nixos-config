...
In awm we keep track of this via the `RedrawState` enum that you can find in an `OutputState`.
...
Then, at the end of an event loop dispatch, awm calls `redraw()` for every `Queued` output.
