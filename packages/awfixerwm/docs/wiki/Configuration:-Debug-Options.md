...
AWM has several options that are only useful for debugging, or are experimental and have known issues.
...
Make awm render the monitors the same way as for a screencast or a screen capture.
...
Force-disables all outputs upon resuming awm (TTY switch or waking up from suspend).
This causes a modeset/screen blank on all outputs.

If awm rendering is corrupted, or monitors don't light up after a TTY switch, you can try this flag.
...
Override the DRM device that awm will use for all rendering.

You can set this to make awm use a different primary GPU than the default one.
...
List DRM devices that awm will ignore.
Useful for GPU passthrough when you don't want awm to open a certain device.
...
Make awm create its D-Bus interfaces even if it's not running as a `--session`.

Useful for testing screencasting changes without having to relogin.

The main awm instance will *not* currently take back the interfaces when you close the test instance, so you will need to relogin in the end to make screencasting work again.
...
This is a thing on NVIDIA proprietary drivers, so this flag can be used to test that awm doesn't break too hard on those systems.
...
Transactions make awm wait until all windows finish resizing before showing them all on screen in one, synchronized frame.
...
By default, awm will disable the internal laptop monitor when the laptop lid is closed.
...
Disables the make/model/serial monitor names, as if awm fails to read them from the EDID.
...
By default, awm ignores xdg-activation tokens with invalid serials, to prevent windows from randomly stealing focus.
This debug flag makes awm honor such tokens, making the aforementioned widely-used apps get focus when clicking on their tray icon or notification.

Use the [`on-xdg-activate` window rule](./Configuration:-Window-Rules.md#on-xdg-activate) to control what awm does for individual windows when it accepts an xdg-activation request.
...
AWM keeps the Activated state on unfocused workspaces and invisible tabbed windows (to reduce unwanted animations), surfacing bugs in these applications.

Set this debug flag to work around these problems.
It will cause awm to drop the Activated state for all unfocused windows.
...
By default, awm will try to output a 10-bit color format to the monitor (before falling back to 8-bit).
