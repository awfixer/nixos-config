There are two main coordinate spaces in awm: physical (pixels of every individual output) and logical (shared among all outputs, takes into account the scale of every output).
...
Thus, awm uses fractional logical coordinates for most of its layout.
...
The way it's handled in awm is:
