...
`Mod` is a special modifier that is equal to `Super` when running awm on a TTY, and to `Alt` when running awm as a nested winit window.
This way, you can test awm in a window without causing too many conflicts with the host compositor's key bindings.
...
>
> When resolving latin keys, awm will search for the *first* configured XKB layout that has the latin key.
...
These binds are also affected by touchpad's `natural-scroll`, so these example binds are "inverted", since awm has `natural-scroll` enabled for touchpads by default.
...
For example, if you have a `Mod+WheelScrollDown` bind, then while holding `Mod`, all mouse wheel scrolling will be consumed by awm.
...
> - If any of the binds has a custom hotkey overlay title, awm will show that bind.
> - Otherwise, if any of the binds has a null title, awm will hide the bind.
> - Otherwise, awm will show the first key combination.
...
Every action that you can bind is also available for programmatic invocation via `awm msg action`.
Run `awm msg action` to get a full list of actions along with their short descriptions.
...
For `spawn`, awm *does not* use a shell to run commands, which means that you need to manually separate arguments.
...
As a special case, awm will expand `~` to the home directory *only* at the beginning of the program name.
...
Exit awm after showing a confirmation dialog to avoid accidentally triggering it.
...
```shell
awm msg action do-screen-transition
...
```shell
awm msg action do-screen-transition --delay-ms 100
```
...
Applications such as remote-desktop clients and software KVM switches may request that awm stops processing its keyboard shortcuts so that they may, for example, forward the key presses as-is to a remote machine.
...
They will always be handled by awm and never passed to the window.
