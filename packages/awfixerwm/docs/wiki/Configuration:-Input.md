...
> <sup>Since: 25.08</sup>
>
> If the `xkb` section is empty (like it is by default), awm will fetch xkb settings from systemd-localed at `org.freedesktop.locale1` over D-Bus.
> This way, for example, system installers can dynamically set the awm keyboard layout.
...
When using multiple layouts, awm can remember the current layout globally (the default) or per-window.
...
By default, awm will take over the power button to make it sleep instead of power off.
...
AWM will correctly switch to the workspace you came from, even if workspaces were reordered in the meantime.
...
By default, `Mod` is equal to `Super` when running awm on a TTY, and to `Alt` when running awm as a nested winit window.
