...
AWM has several animations which you can configure in the same way.
...
Currently, awm only supports five curves.
...
If a custom shader fails to compile, awm will print a warning and fall back to the default, or previous successfully compiled shader.
When running awm as a systemd service, you can see the warnings in the journal: `journalctl -ef /usr/bin/awm`
    ...
    window-open {
        ...
        custom-shader r"
            vec4 open_color(vec3 coords_geo, vec3 size_geo) {
                ...
                return color * awm_clamped_progress;
            }
        "
    }
}
    ...
    window-close {
        custom-shader r"
            vec4 close_color(vec3 coords_geo, vec3 size_geo) {
                ...
                return color * (1.0 - awm_clamped_progress);
            }
        "
    }
}
...
Sometimes, when two animations are meant to play together synchronized, awm will drive them both with the same configuration.
