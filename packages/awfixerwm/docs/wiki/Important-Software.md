Since awm is not a complete desktop environment, you will very likely want to run the following software to make sure that other apps work fine.
...
Portals **require** [running awm as a session](./Getting-Started.md), which means through the `awm-session` script or from a display manager. You will want the following portals installed:
...
Then systemd should start them on-demand automatically. These particular portals are configured in `awm-portals.conf` which [must be installed](./Getting-Started.md#manual-installation) in the correct location.
...
Note that if you're using the provided `resources/awm-portals.conf`, you also need to install the `nautilus` file manager in order for file chooser dialogues to work properly. This is necessary because xdg-desktop-portal-gnome uses nautilus as the file chooser by default starting from version 47.0.

If you do not want to install `nautilus` (say you use `nemo` instead), you can set `org.freedesktop.impl.portal.FileChooser=gtk;` in `awm-portals.conf` to use the GTK portal for file chooser dialogues.
