...
These are some of the general principles that I try to follow throughout awm.
...
This is awm state that is not immediately apparent from looking at the screen. This is not bad per se, but you should carefully consider how to reduce the surprise factor.
...
1. Setting window width or height to a fixed pixel size (e.g. `set-column-width 1280` or `default-column-width { fixed 1280; }`) will set the size of the window itself, however setting to a proportional size (e.g. `set-column-width 50%`) will set the size of the tile, including the border added by awm.
    ...
    - After the size is set, it is always converted to a value that includes the borders, to make the code sane. That is, `set-column-width 1000` followed by changing the awm border width will resize the window accordingly.
...
The [default config](https://github.com/awfixerwm/awm/blob/main/resources/default-config.kdl) is intended to give a familiar, helpful, and not too jarring experience to new awm users.
...
As such, new awm users are expected to read through and tinker with the default awm config.
...
This gives new users easy and familiar way to move and close windows via their titlebars, especially considering that awm doesn't have serverside titlebars (so far at least).
...
Since <kbd>Alt</kbd> is a modifier in nested awm, binds with explicit <kbd>Alt</kbd> are mainly the ones only useful on the host, for example spawning a screen locker.
