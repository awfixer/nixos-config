...
There's [a bug](https://github.com/wezterm/wezterm/issues/4708) in WezTerm that it waits for a zero-sized Wayland configure event, so its window never shows up in awm. To work around it, put this window rule in the awm config (included in the default config):
...
AWM puts windows in the tiled state with [`prefer-no-csd`](./Configuration:-Miscellaneous.md#prefer-no-csd).
So if you hit this problem, comment out `prefer-no-csd` in the awm config and restart WezTerm.
...
For some reason, DMABUF screencasts are disabled in the Zen Browser, so screencasting doesn't work out of the box on awm.
...
Note that the awm environment config does not propagate to apps and shells started by systemd, for example to DankMaterialShell and its application launcher.
You can set the variable in your login shell config (i.e. `~/.bash_profile`) instead, though keep in mind that then it will be set for all compositors, not just awm.
...
You can move them to a more convenient location by adding a window rule in your awm config:
...
This leads to awm filling the transparent pixels inside the corners with black.
