...
You can find the title and the app ID of a window by running `awm msg pick-window` and clicking on the window in question.
...
Can be `true` or `false`.
Matches during the first 60 seconds after starting awm.

This is useful for properties like `open-on-output` which you may want to apply only right after starting awm.

```kdl
// Open windows on the HDMI-A-1 monitor at awm startup, but not afterwards.
window-rule {
    match at-startup=true
    open-on-output "HDMI-A-1"
}
...
To be precise, they apply at the point when awm sends the initial configure request to the window.
...
Set what awm does when a window requests activation through [xdg-activation](https://wayland.app/protocols/xdg-activation-v1).
...
Requests with a set, but invalid, serial, are always ignored, which you can change with the [`honor-xdg-activation-with-invalid-serial`](https://awfixerwm.github.io/awm/Configuration%3A-Debug-Options.html#honor-xdg-activation-with-invalid-serial) debug flag.
...
By default, awm will set the tiled state to `true` together with [`prefer-no-csd`](./Configuration:-Miscellaneous.md#prefer-no-csd) in order to improve behavior for apps that don't support server-side decorations.
...
> Subsurfaces are an indivisible part of a surface and they aren't usually pop-ups, so it wouldn't make sense for awm to apply these rules to them.
>
...
These values instruct awm to never ask the window to be smaller than the minimum you set, or to be bigger than the maximum you set.
...
>
> This is a limitation of awm's window height distribution algorithm.
