...
If set, awm will always center a single column on a workspace, regardless of the `center-focused-column` option.
...
If set, awm will always add an empty workspace at the very start, in addition to the empty workspace at the very end.
...
> [!NOTE]
> `default-column-width {}` causes awm to send a (0, H) size in the initial configure request.
>
...
Note that awm has no way of knowing about the CSD window corner radius.
...
Then, awm will know the corner radius and draw the shadow correctly, without having to draw it behind the window.
...
1. If neither are set, awm picks the color matching the window border or focus ring, whichever one is active.
...
Set the default background color that awm draws for workspaces.
