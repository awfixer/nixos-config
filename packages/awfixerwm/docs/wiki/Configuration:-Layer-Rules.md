...
You can find the namespaces of all open layer-shell surfaces by running `awm msg layers`.
...
```kdl
// Show layer-shell surfaces with 0.5 opacity at awm startup, but not afterwards.
layer-rule {
    match at-startup=true
...
> [!NOTE]
> Layer surfaces have no way to tell awm about their *visual geometry*.
> For example, if a layer surface includes some invisible margins (like mako), awm has no way of knowing that, and will draw the shadow behind the entire surface, including the invisible margins.
>
> So to use awm shadows, you'll need to configure layer-shell clients to remove their own margins or shadows.
...
> As far as awm is concerned, those are just layer surfaces and not pop-ups, so this block won't apply to them.
>
