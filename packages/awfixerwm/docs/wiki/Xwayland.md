...
With no further configuration, awm will create X11 sockets on disk, export `$DISPLAY`, and spawn xwayland-satellite on-demand when an X11 client connects.
If xwayland-satellite dies, awm will automatically restart it.
...
To check that the integration works, verify that the awm output says something like `listening on X11 socket: :0`:

```sh
$ journalctl --user-unit=awm -b
systemd[2338]: Starting awm.service - A scrollable-tiling Wayland compositor...
awm[2474]: 2025-08-29T04:07:40.043402Z  INFO awm: starting version 25.05.1 (0.0.git.2345.d9833fc1)
(...)
awm[2474]: 2025-08-29T04:07:40.690512Z  INFO awm: listening on Wayland socket: wayland-1
awm[2474]: 2025-08-29T04:07:40.690520Z  INFO awm: IPC listening on: /run/user/1000/awm.wayland-1.2474.sock
awm[2474]: 2025-08-29T04:07:40.700137Z  INFO awm: listening on X11 socket: :0
systemd[2338]: Started awm.service - A scrollable-tiling Wayland compositor.
$ echo $DISPLAY
:0
```
...
We're using xwayland-satellite rather than Xwayland directly because [X11 is very cursed](./FAQ.md#why-doesnt-awm-integrate-xwayland-like-other-compositors).
xwayland-satellite takes on the bulk of the work dealing with the X11 peculiarities from us, giving awm normal Wayland windows to manage.
...
1. Run it inside awm with the `labwc` command.
...
![Xwayland running in rootful mode.](https://github.com/awfixerwm/awm/assets/1794388/b64e96c4-a0bb-4316-94a0-ff445d4c7da7)
...
- `env DISPLAY=:0 xsel -ob | wl-copy` to copy from Xwayland to awm clipboard
- `wl-paste -n | env DISPLAY=:0 xsel -ib` to copy from awm to Xwayland clipboard
...
2. Run `cage -- /path/to/application` and enjoy your X11 program on awm.
