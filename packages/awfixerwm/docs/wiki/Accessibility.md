...
AWM has basic support for screen readers (specifically, [Orca](https://orca.gnome.org)) when running as a full desktop session, i.e. you need to start awm through a display manager or through `awm-session`.
To avoid conflicts with an already running compositor, awm won't expose accessibility interfaces when started as a nested window, or as a plain `/usr/bin/awm` on a TTY.

We implement the `org.freedesktop.a11y.KeyboardMonitor` D-Bus interface for Orca to listen and grab keyboard keys, and we expose the main awm UI elements via [AccessKit](https://accesskit.dev).
Specifically, awm will announce:
...
- <sup>Since: 25.11</sup> awm has an <kbd>Alt</kbd><kbd>Tab</kbd> window switcher where it will announce the selected window title;
- entering the screenshot UI and the overview (awm will say when these are focused, nothing else for now);
...
- You need to have a screen connected and enabled. Without a screen, awm won't give focus any window. This makes sense for sighted users, and I'm not entirely sure what makes the most sense for accessibility purposes (maybe, it'd be better solved with virtual monitors).
- You need working EGL (hardware acceleration).
- We don't have screen curtain functionality yet.

If you're shipping awm and would like to make it work better for screen readers out of the box, consider the following changes to the default awm config:
...
- Add some [`spawn-at-startup`](./Configuration:-Miscellaneous.md#spawn-at-startup) command that plays a sound which will indicate to users that awm has finished loading.
- Add `spawn-at-startup "orca"` to run Orca automatically at awm startup.
