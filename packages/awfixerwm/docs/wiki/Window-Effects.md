...
You can also enable blur on the awm side with the `blur true` background effect window rule:
...
If the window or layer has clientside rounded corners or other complex shape, it should set a corresponding blur shape through `ext-background-effect`, then it will get correctly shaped background blur without any manual awm configuration.

Windows can also blur their pop-up menus using `ext-background-effect`.
On the awm side, you can do it with a `popups` block inside [`window-rule`](./Configuration:-Window-Rules.md#popups) and [`layer-rule`](./Configuration:-Layer-Rules.md#popups).
...
This is because it's much more efficient: with xray active, awm only needs to blur the background once, and then can reuse this blurred version with no extra work (since the wallpaper changes very rarely).
...
Keep in mind that non-xray blur and other non-xray effects are more expensive as awm has to recompute them any time you move the window, or the contents underneath change.
...
> Fixing this requires a refactor to the awm rendering code to defer offscreen rendering, and possibly other refactors.
