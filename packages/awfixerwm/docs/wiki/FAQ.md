...
Then awm will ask windows to omit client-side decorations, and also inform them that they are being tiled (which makes some windows rectangular, even if they cannot omit the decorations).
...
Uncomment the [`prefer-no-csd` setting](./Configuration:-Miscellaneous.md#prefer-no-csd) at the top level of the config, and then restart your apps.
AWM will draw focus rings and borders *around* windows that agree to omit their client-side decorations.
...
However, this means that awm has to render the external monitor contents on the integrated GPU, then copy them over to the discrete GPU for display.
On some laptops this can cause lag and stuttering (it gets worse with monitor resolution and refresh rate).

If your laptop has a MUX switch—usually a GPU toggle in the UEFI settings—then you can switch it to use the discrete GPU, then awm will render on the discrete GPU, and the external monitor won't lag.
Otherwise, you can try configuring awm to render on the discrete GPU via the [`render-drm-device`](./Configuration:-Debug-Options.md#render-drm-device) debug option.
...
### Why doesn't awm integrate Xwayland like other compositors?

A combination of factors:

- Integrating Xwayland is quite a bit of work, as the compositor needs to implement parts of an X11 window manager.
- You need to appease the X11 ideas of windowing, whereas for awm I want to have the best code for Wayland.
- awm doesn't have a good global coordinate system required by X11.
...
- awm isn't a Big Serious Desktop Environment which Must Support All Use Cases (and is Backed By Some Corporation).

All in all, the situation works out in favor of avoiding Xwayland integration.

<sup>Since: 25.08</sup> awm has seamless built-in xwayland-satellite integration that by and large works as well as built-in Xwayland in other compositors, solving the hurdle of having to set it up manually.
...
Not yet, follow/upvote [this issue](https://github.com/awfixerwm/awm/issues/932).

You can emulate this with a script that uses the awm IPC.
For example, [awmus](https://git.sr.ht/~tsdh/awmus) seems to have this feature (`toggle-follow-mode`).
...
You'll need to use a script, for example [this one](https://github.com/awfixerwm/awm/discussions/1599) or other ones (search awm issues and discussions for Bitwarden).

### Can I open a window directly in the current column / in the same column as another window?

No, but you can script the behavior you want with the [awm IPC](./IPC.md).
Listen to the event stream for a new window opening, then call an action like `consume-or-expel-window-left`.

Adding this directly to awm is challenging:

- The act of "opening a window directly in some column" by itself is quite involved. AWM will have to compute the exact initial window size provided how other windows in a column would resize in response. This logic exists, but it isn't directly pluggable to the code computing a size for a new window. Then, it'll need to handle all sorts of edge cases like the column disappearing, or new windows getting added to the column, before the target window had a chance to appear.
- How do you indicate if a new window should spawn in an existing column (and in which one), as opposed to a new column? Different people seem to have different needs here (including very complex rules based on parent PID, etc.), and it's very unclear design-wise what kind of (simple) setting is actually needed and would be useful. See also https://github.com/awfixerwm/awm/discussions/1125.
...
This is awm's locked session background.

You can recover from this by spawning a new screen locker.
One way is to switch to a different TTY (with a shortcut like <kbd>Ctrl</kbd><kbd>Alt</kbd><kbd>F3</kbd>) and spawning a screen locker to awm's Wayland display, e.g. `WAYLAND_DISPLAY=wayland-1 swaylock`.
...
Kanshi has its own simple configuration and communicates with awm via IPC. You may want to launch kanshi from the awm config.kdl e.g. `spawn-at-startup "/usr/bin/kanshi"`
...
They draw their own 1 px dark border around the window, which obscures one pixel of awm's border.
