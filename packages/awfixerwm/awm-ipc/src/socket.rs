//! Helper for blocking communication over the awm socket.
...
/// Name of the environment variable containing the awm IPC socket path.
pub const SOCKET_PATH_ENV: &str = "AWM_SOCKET";

/// Helper for blocking communication over the awm socket.
///
/// This struct is used to communicate with the awm IPC server. It handles the socket connection
...
impl Socket {
    /// Connects to the default awm IPC socket.
    ///
    ...
    pub fn connect() -> io::Result<Self> {
        let socket_path = env::var_os(SOCKET_PATH_ENV).ok_or_else(|| {
            io::Error::new(
                ...
                format!("{SOCKET_PATH_ENV} is not set, are you running this within awm?"),
            )
        })?;
        Self::connect_to(socket_path)
    }

    /// Connects to the awm IPC socket at the given path.
    ...
    /// Sends a request to awm and returns the response.
    ///
    /// Return values:
    ///
    /// * `Ok(Ok(response))`: successful [`Response`](crate::Response) from awm
    /// * `Ok(Err(message))`: error message from awm
    /// * `Err(error)`: error communicating with awm
