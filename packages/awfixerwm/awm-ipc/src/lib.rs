//! Types for communicating with awm via IPC.
//!
//! After connecting to the awm socket, you can send [`Request`]s. AWM will process them one by
//! one, in order, and to each request it will respond with a single [`Reply`], which is a `Result`
//! wrapping a [`Response`].
//!
//! If you send a [`Request::EventStream`], awm will *stop* reading subsequent [`Request`]s, and
...
//!
//! 1. Read the socket filesystem path from [`socket::SOCKET_PATH_ENV`] (`$AWM_SOCKET`).
...
//! 3. AWM will respond with a single line JSON-formatted [`Reply`].
//! 4. You can keep writing [`Request`]s, each on a single line, and read [`Reply`]s, also each on a
//!    separate line.
//! 5. After you request an event stream, awm will keep responding with JSON-formatted [`Event`]s,
//!    on a single line each.
//!
//! ## Backwards compatibility
//!
//! This crate follows the awm version. It is **not** API-stable in terms of the Rust semver. In
...
//!   the types.
//! - `clap`: derives the clap CLI parsing traits for some types. Used internally by awm itself.
#![warn(missing_docs)]
...
/// Request from client to awm.
#[derive(Debug, Serialize, Deserialize, Clone)]
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum Request {
    /// Request the version string for the running awm instance.
    Version,
...
/// Successful response from awm to client.
#[derive(Debug, Serialize, Deserialize, Clone)]
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum Response {
    /// A request that does not need a response was handled successfully.
    Handled,
    /// The version string for the running awm instance.
    Version(String),
...
/// Actions that awm can perform.
// Variants in this enum should match the spelling of the ones in awm-config. Most, but not all,
// variants from awm-config should be present here.
#[derive(Serialize, Deserialize, Debug, Clone)]
...
pub enum Action {
    ...
    /// awm's config file watcher to notice the changes.
    LoadConfigFile {
...
/// Output actions that awm can perform.
// Variants in this enum should match the spelling of the ones in awm-config. Most thigs from
// awm-config should be present here.
#[derive(Serialize, Deserialize, Debug, Clone)]
#[cfg_attr(feature = "clap", derive(clap::Parser))]
#[cfg_attr(feature = "clap", command(subcommand_value_name = "ACTION"))]
#[cfg_attr(feature = "clap", command(subcommand_help_heading = "Actions"))]
...
pub enum OutputAction {
    ...
    Mode {
        ...
        ///
        /// Run `awm msg outputs` to see the available modes.
        #[cfg_attr(feature = "clap", arg())]
        mode: ModeToSet,
    },
...
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum ModeToSet {
    /// AWM will pick the mode automatically.
    Automatic,
    /// Specific mode.
...
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum ScaleToSet {
    /// AWM will pick the scale automatically.
    Automatic,
    /// Specific scale.
    Specific(f64),
}
...
/// some optional properties are present or absent for certain window types may change across awm
/// releases.
///
...
pub struct WindowLayout {
    ...
    ///
    /// Does not include awm decorations like borders.
    ///
    ...
    ///
    /// This is the same "workspace view" as in gradients' `relative-to` in the awm config.
...
pub struct Workspace {
    ...
    ///
    /// This is the same index you can use for requests like `awm msg action focus-workspace`.
    ///
