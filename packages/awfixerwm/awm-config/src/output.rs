...
use knuffel::Decode;
use awm_ipc::{ConfiguredMode, HSyncPolarity, Transform, VSyncPolarity};
...
pub struct Modeline {
    ...
    pub hsync_polarity: awm_ipc::HSyncPolarity,
    /// Vertical sync polarity: "+vsync" or "-vsync".
    pub vsync_polarity: awm_ipc::VSyncPolarity,
}
...
pub struct MaxBpc(pub awm_ipc::MaxBpc);
...
impl OutputName {
    pub fn from_ipc_output(output: &awm_ipc::Output) -> Self {
        Self {
    ...
    pub fn matches(&self, target: &str) -> bool {
        ...
        //
        // This is used by awm msg output.
        if self.make.is_none() && self.model.is_none() && self.serial.is_none() {
            return false;
        }
    ...
    fn raw_decode(
        ...
        match &**value {
            knuffel::ast::Literal::Int(ref val) => match u8::try_from(val) {
                Ok(v) => awm_ipc::MaxBpc::try_from(v)
                    .map(MaxBpc)
