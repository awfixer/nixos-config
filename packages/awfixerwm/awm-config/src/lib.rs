...
pub enum ConfigPath {
    ...
    Regular {
        /// User config path, usually `$XDG_CONFIG_HOME/awm/config.kdl`.
        user_path: PathBuf,
        /// System config path, usually `/etc/awm/config.kdl`.
        system_path: PathBuf,
    },
}
...
mod tests {
    ...
    fn diff_empty_to_default() {
        ...
        // config) give the same outcome as the default config bundled with awm. This test
